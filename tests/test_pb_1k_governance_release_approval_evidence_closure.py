...
from runtime.computer_use.pb_1j_governance_chain_release_readiness_contract import READY, READY_WITH_RESTRICTIONS, BLOCKED
from runtime.computer_use.pb_1k_governance_release_approval_evidence_closure import (
    CAPABILITY_NAME,
    REQUIRED_VALIDATION_CHECKS,
    canonical_pb_1k_json,
    evaluate_pb_1k_approval_evidence_closure,
    expected_pb_1k_approval_closure_hash,
    expected_pb_1k_evidence_hash,
    expected_pb_1k_package_hash,
)
...
def _approval(**overrides):
    record = {
        ...
        "pb_1j_decision_hash": HASH_D,
        "pb_1j_package_hash": HASH_C,
...
def _audit(**overrides):
    record = {
        "pb_1j_audit_hash": HASH,
        "pb_1k_validation_audit_hash": HASH_B,
...
def _evidence(**overrides):
    record = {
        "pb_1j_evidence_reference": HASH_B,
        "pb_1j_audit_reference": HASH,
...
def _payload(**overrides):
    ...
    payload["expected_approval_closure_hash"] = expected_pb_1k_approval_closure_hash(payload)
    payload["expected_evidence_hash"] = expected_pb_1k_evidence_hash(payload)
    payload["expected_package_hash"] = expected_pb_1k_package_hash(payload)
...
def _blocked(payload, reason):
    result = evaluate_pb_1k_approval_evidence_closure(payload)
...
def test_valid_approval_evidence_closure_ready() -> None:
    result = evaluate_pb_1k_approval_evidence_closure(_payload())

    assert result.final_decision == READY
    assert result.reason_codes == ()


def test_valid_closure_with_governed_restrictions() -> None:
    payload = _payload(
        pb_1j=_pb_1j(final_decision=READY_WITH_RESTRICTIONS),
    ...
    assert evaluate_pb_1k_approval_evidence_closure(payload).final_decision == READY_WITH_RESTRICTIONS


def test_missing_pb_1j_metadata_blocks() -> None:
    _blocked(_payload(pb_1j=None), "PB_1J_METADATA_MISSING")


def test_invalid_pb_1j_decision_blocks() -> None:
    _blocked(_payload(pb_1j=_pb_1j(final_decision="ALLOW")), "INVALID_PB_1J_DECISION")


def test_missing_pb_1j_hashes_block() -> None:
    _blocked(_payload(pb_1j=_pb_1j(contract_hash="")), "PB_1J_CONTRACT_HASH_MISSING")
...
def test_duplicated_approval_reference_blocks() -> None:
    _blocked(_payload(approval_chain=_approval(duplicate=True)), "DUPLICATE_APPROVAL_METADATA")
...
def test_missing_audit_reference_blocks() -> None:
    _blocked(_payload(audit_chain=None), "AUDIT_CHAIN_MISSING")


def test_invalid_audit_reference_blocks() -> None:
...
def test_duplicate_package_metadata_blocks() -> None:
    _blocked(_payload(replay_protection=_replay(duplicate_package_detected=True)), "DUPLICATE_APPROVAL_METADATA")
...
def test_invalid_rollback_evidence_blocks() -> None:
    _blocked(_payload(rollback_evidence=_rollback(status="MISSING")), "ROLLBACK_EVIDENCE_MISSING")
...
def test_mismatched_policy_reference_blocks() -> None:
...
def test_stale_metadata_blocks() -> None:
    _blocked(_payload(pb_1j=_pb_1j(metadata_stale=True)), "STALE_METADATA")
...
def test_upstream_blocked_propagates() -> None:
    _blocked(_payload(pb_1j=_pb_1j(final_decision=BLOCKED)), "UPSTREAM_BLOCKED")


def test_hash_mismatch_blocks() -> None:
    _blocked(_payload(expected_approval_closure_hash=HASH), "APPROVAL_CLOSURE_HASH_MISMATCH")
...
def test_malformed_metadata_blocks() -> None:
    _blocked(None, "MALFORMED_PB_1K_METADATA")


def test_deterministic_repeated_evaluation() -> None:
    payload = _payload()

    assert evaluate_pb_1k_approval_evidence_closure(payload).to_dict() == evaluate_pb_1k_approval_evidence_closure(payload).to_dict()


def test_canonical_decision_state_and_reason_order() -> None:
    result = evaluate_pb_1k_approval_evidence_closure(_payload(pb_1j=None, approval_chain=None))
...
def test_canonical_json_is_stable() -> None:
    decision = evaluate_pb_1k_approval_evidence_closure(_payload())

    assert canonical_pb_1k_json(decision) == canonical_pb_1k_json(decision)
...
def test_false_execution_flags_and_redacted_output() -> None:
    ...
    assert payload["approval_closure_hash"].startswith("sha256:")

...
from runtime.computer_use.pb_1m_governance_final_release_review_package import (
    CAPABILITY_NAME,
    REQUIRED_VALIDATION_CHECKS,
    canonical_pb_1m_json,
    evaluate_pb_1m_final_release_review_package,
    expected_pb_1m_evidence_hash,
    expected_pb_1m_package_hash,
    expected_pb_1m_final_review_package_hash,
)
...
def _pb_1l(**overrides):
    record = {
        "final_decision": READY,
        "release_candidate_lock_hash": HASH,
...
def _final_review(**overrides):
    record = {
        "final_review_reference": HASH,
        "final_review_evidence_hash": HASH_B,
        "expected_final_review_evidence_hash": HASH_B,
        "pb_1l_decision_hash": HASH_D,
        "pb_1l_package_hash": HASH_C,
...
def _evidence(**overrides):
    record = {
        "pb_1l_release_candidate_lock_reference": HASH,
        "pb_1l_evidence_reference": HASH_B,
        "pb_1l_audit_reference": HASH,
        "final_review_evidence_reference": HASH_B,
...
def _rollback(**overrides):
    record = {
        ...
        "rollback_evidence_reference": HASH_C,
        "previous_final_review_package_hash": HASH_B,
...
def _replay(**overrides):
    record = {
        ...
        "duplicate_final_review_detected": False,
...
def _payload(**overrides):
    ...
    payload["expected_final_review_package_hash"] = expected_pb_1m_final_review_package_hash(payload)
    payload["expected_evidence_hash"] = expected_pb_1m_evidence_hash(payload)
    payload["expected_package_hash"] = expected_pb_1m_package_hash(payload)
    payload["rollback_references"]["current_final_review_package_hash"] = payload["expected_package_hash"]
...
def _blocked(payload, reason):
    result = evaluate_pb_1m_final_release_review_package(payload)
...
def test_valid_final_release_review_package_ready() -> None:
    result = evaluate_pb_1m_final_release_review_package(_payload())
...
def test_valid_lock_with_governed_restrictions() -> None:
    payload = _payload(
        pb_1l=_pb_1l(final_decision=READY_WITH_RESTRICTIONS),
    ...
    assert evaluate_pb_1m_final_release_review_package(payload).final_decision == READY_WITH_RESTRICTIONS


def test_missing_pb_1l_metadata_blocks() -> None:
    _blocked(_payload(pb_1l=None), "PB_1L_METADATA_MISSING")


def test_invalid_pb_1l_decision_blocks() -> None:
    _blocked(_payload(pb_1l=_pb_1l(final_decision="ALLOW")), "INVALID_PB_1L_DECISION")


def test_missing_pb_1l_hashes_block() -> None:
    _blocked(_payload(pb_1l=_pb_1l(release_candidate_lock_hash="")), "PB_1L_RELEASE_CANDIDATE_LOCK_HASH_MISSING")


def test_missing_final_review_reference_blocks() -> None:
    _blocked(_payload(final_review=None), "FINAL_REVIEW_REFERENCE_MISSING")


def test_missing_final_review_evidence_blocks() -> None:
    _blocked(_payload(final_review=_final_review(expected_final_review_evidence_hash=HASH)), "FINAL_REVIEW_EVIDENCE_MISSING")
...
def test_duplicate_approval_metadata_blocks() -> None:
    _blocked(_payload(approval_chain=_approval(duplicate=True)), "DUPLICATE_FINAL_REVIEW_METADATA")


def test_duplicate_final_review_metadata_blocks() -> None:
    _blocked(_payload(final_review=_final_review(duplicate=True)), "DUPLICATE_FINAL_REVIEW_METADATA")


def test_duplicate_package_metadata_blocks() -> None:
    _blocked(_payload(replay_protection=_replay(duplicate_package_detected=True)), "DUPLICATE_FINAL_REVIEW_METADATA")
...
def test_stale_metadata_blocks() -> None:
    _blocked(_payload(pb_1l=_pb_1l(metadata_stale=True)), "STALE_METADATA")
...
def test_upstream_blocked_propagates() -> None:
    _blocked(_payload(pb_1l=_pb_1l(final_decision=BLOCKED)), "UPSTREAM_BLOCKED")


def test_hash_mismatch_blocks() -> None:
    _blocked(_payload(expected_final_review_package_hash=HASH), "FINAL_REVIEW_PACKAGE_HASH_MISMATCH")
...
def test_malformed_metadata_blocks() -> None:
    _blocked(None, "MALFORMED_PB_1M_METADATA")


def test_deterministic_repeated_evaluation() -> None:
    payload = _payload()

    assert evaluate_pb_1m_final_release_review_package(payload).to_dict() == evaluate_pb_1m_final_release_review_package(payload).to_dict()


def test_canonical_decision_state_and_reason_order() -> None:
    result = evaluate_pb_1m_final_release_review_package(_payload(pb_1l=None, approval_chain=None))
...
def test_canonical_json_is_stable() -> None:
    decision = evaluate_pb_1m_final_release_review_package(_payload())

    assert canonical_pb_1m_json(decision) == canonical_pb_1m_json(decision)
...
def test_false_execution_flags_and_redacted_output() -> None:
    ...
    assert payload["final_review_package_hash"].startswith("sha256:")

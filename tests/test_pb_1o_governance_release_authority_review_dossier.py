...
from runtime.computer_use.pb_1o_governance_release_authority_review_dossier import (
    CAPABILITY_NAME,
    REQUIRED_VALIDATION_CHECKS,
    canonical_pb_1o_release_authority_review_dossier_json,
    evaluate_pb_1o_release_authority_review_dossier,
    expected_pb_1o_evidence_hash,
    expected_pb_1o_package_hash,
    expected_pb_1o_release_authority_review_dossier_hash,
)
...
def _pb_1n(**overrides):
    record = {
        "final_decision": READY,
        "release_authorization_readiness_handoff_hash": HASH,
...
def _release_authority_review(**overrides):
    record = {
        "release_authority_review_reference": HASH,
        "release_authority_review_evidence_hash": HASH_B,
        "expected_release_authority_review_evidence_hash": HASH_B,
        "pb_1n_decision_hash": HASH_D,
        "pb_1n_package_hash": HASH_C,
...
def _evidence(**overrides):
    record = {
        "pb_1n_release_authorization_readiness_handoff_reference": HASH,
        "pb_1n_evidence_reference": HASH_B,
        "pb_1n_audit_reference": HASH,
        "release_authority_review_evidence_reference": HASH_B,
...
def _rollback(**overrides):
    record = {
        ...
        "rollback_evidence_reference": HASH_C,
        "previous_release_authority_review_dossier_hash": HASH_B,
...
def _replay(**overrides):
    record = {
        ...
        "duplicate_release_authority_review_detected": False,
...
def _payload(**overrides):
    payload = {
        "pb_1n": _pb_1n(),
        "release_authority_review": _release_authority_review(),
    ...
    payload["expected_release_authority_review_dossier_hash"] = expected_pb_1o_release_authority_review_dossier_hash(payload)
    payload["expected_evidence_hash"] = expected_pb_1o_evidence_hash(payload)
    payload["expected_package_hash"] = expected_pb_1o_package_hash(payload)
    payload["rollback_references"]["current_release_authority_review_dossier_hash"] = payload["expected_package_hash"]
...
def _blocked(payload, reason):
    result = evaluate_pb_1o_release_authority_review_dossier(payload)
...
def test_valid_release_authority_review_dossier_ready() -> None:
    result = evaluate_pb_1o_release_authority_review_dossier(_payload())
...
def test_valid_lock_with_governed_restrictions() -> None:
    payload = _payload(
        pb_1n=_pb_1n(final_decision=READY_WITH_RESTRICTIONS),
    ...
    assert evaluate_pb_1o_release_authority_review_dossier(payload).final_decision == READY_WITH_RESTRICTIONS


def test_missing_pb_1n_metadata_blocks() -> None:
    _blocked(_payload(pb_1n=None), "PB_1N_METADATA_MISSING")


def test_invalid_pb_1n_decision_blocks() -> None:
    _blocked(_payload(pb_1n=_pb_1n(final_decision="ALLOW")), "INVALID_PB_1N_DECISION")


def test_missing_pb_1n_hashes_block() -> None:
    _blocked(_payload(pb_1n=_pb_1n(release_authorization_readiness_handoff_hash="")), "PB_1N_RELEASE_AUTHORIZATION_READINESS_HANDOFF_HASH_MISSING")


def test_missing_release_authority_review_reference_blocks() -> None:
    _blocked(_payload(release_authority_review=None), "RELEASE_AUTHORITY_REVIEW_REFERENCE_MISSING")


def test_missing_release_authority_review_evidence_blocks() -> None:
    _blocked(_payload(release_authority_review=_release_authority_review(expected_release_authority_review_evidence_hash=HASH)), "RELEASE_AUTHORITY_REVIEW_EVIDENCE_MISSING")
...
def test_duplicate_approval_metadata_blocks() -> None:
    _blocked(_payload(approval_chain=_approval(duplicate=True)), "DUPLICATE_RELEASE_AUTHORITY_REVIEW_METADATA")


def test_duplicate_release_authority_review_metadata_blocks() -> None:
    _blocked(_payload(release_authority_review=_release_authority_review(duplicate=True)), "DUPLICATE_RELEASE_AUTHORITY_REVIEW_METADATA")


def test_duplicate_package_metadata_blocks() -> None:
    _blocked(_payload(replay_protection=_replay(duplicate_package_detected=True)), "DUPLICATE_RELEASE_AUTHORITY_REVIEW_METADATA")
...
def test_stale_metadata_blocks() -> None:
    _blocked(_payload(pb_1n=_pb_1n(metadata_stale=True)), "STALE_METADATA")
...
def test_upstream_blocked_propagates() -> None:
    _blocked(_payload(pb_1n=_pb_1n(final_decision=BLOCKED)), "UPSTREAM_BLOCKED")


def test_hash_mismatch_blocks() -> None:
    _blocked(_payload(expected_release_authority_review_dossier_hash=HASH), "RELEASE_AUTHORITY_REVIEW_DOSSIER_HASH_MISMATCH")
...
def test_malformed_metadata_blocks() -> None:
    _blocked(None, "MALFORMED_PB_1O_METADATA")


def test_deterministic_repeated_evaluation() -> None:
    payload = _payload()

    assert evaluate_pb_1o_release_authority_review_dossier(payload).to_dict() == evaluate_pb_1o_release_authority_review_dossier(payload).to_dict()


def test_canonical_decision_state_and_reason_order() -> None:
    result = evaluate_pb_1o_release_authority_review_dossier(_payload(pb_1n=None, approval_chain=None))
...
def test_canonical_json_is_stable() -> None:
    decision = evaluate_pb_1o_release_authority_review_dossier(_payload())

    assert canonical_pb_1o_release_authority_review_dossier_json(decision) == canonical_pb_1o_release_authority_review_dossier_json(decision)
...
def test_false_execution_flags_and_redacted_output() -> None:
    ...
    assert payload["release_authority_review_dossier_hash"].startswith("sha256:")

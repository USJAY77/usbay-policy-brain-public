...
from runtime.computer_use.pb_1r_governance_release_successor_authority_handoff import (
    CAPABILITY_NAME,
    REQUIRED_VALIDATION_CHECKS,
    canonical_pb_1r_release_successor_authority_handoff_json,
    evaluate_pb_1r_release_successor_authority_handoff,
    expected_pb_1r_evidence_hash,
    expected_pb_1r_package_hash,
    expected_pb_1r_release_successor_authority_handoff_hash,
)
...
def _pb_1q(**overrides):
    record = {
        "final_decision": READY,
        "release_continuity_review_hash": HASH,
...
def _release_successor_authority_handoff(**overrides):
    record = {
        "release_successor_authority_handoff_reference": HASH,
        "release_successor_authority_handoff_evidence_hash": HASH_B,
        "expected_release_successor_authority_handoff_evidence_hash": HASH_B,
        "pb_1q_decision_hash": HASH_D,
        "pb_1q_package_hash": HASH_C,
...
def _evidence(**overrides):
    record = {
        "pb_1q_release_continuity_review_reference": HASH,
        "pb_1q_evidence_reference": HASH_B,
        "pb_1q_audit_reference": HASH,
        "release_successor_authority_handoff_evidence_reference": HASH_B,
...
def _rollback(**overrides):
    record = {
        ...
        "rollback_evidence_reference": HASH_C,
        "previous_release_successor_authority_handoff_hash": HASH_B,
...
def _replay(**overrides):
    record = {
        ...
        "duplicate_successor_authority_handoff_detected": False,
...
def _payload(**overrides):
    payload = {
        "pb_1q": _pb_1q(),
        "release_successor_authority_handoff": _release_successor_authority_handoff(),
    ...
    payload["expected_release_successor_authority_handoff_hash"] = expected_pb_1r_release_successor_authority_handoff_hash(payload)
    payload["expected_evidence_hash"] = expected_pb_1r_evidence_hash(payload)
    payload["expected_package_hash"] = expected_pb_1r_package_hash(payload)
    payload["rollback_references"]["current_release_successor_authority_handoff_hash"] = payload["expected_package_hash"]
...
def _blocked(payload, reason):
    result = evaluate_pb_1r_release_successor_authority_handoff(payload)
...
def test_valid_release_successor_authority_handoff_ready() -> None:
    result = evaluate_pb_1r_release_successor_authority_handoff(_payload())
...
def test_valid_packet_with_governed_restrictions() -> None:
    payload = _payload(
        pb_1q=_pb_1q(final_decision=READY_WITH_RESTRICTIONS),
    ...
    assert evaluate_pb_1r_release_successor_authority_handoff(payload).final_decision == READY_WITH_RESTRICTIONS


def test_missing_pb_1q_metadata_blocks() -> None:
    _blocked(_payload(pb_1q=None), "PB_1Q_METADATA_MISSING")


def test_invalid_pb_1q_decision_blocks() -> None:
    _blocked(_payload(pb_1q=_pb_1q(final_decision="ALLOW")), "INVALID_PB_1Q_DECISION")


def test_missing_pb_1q_hashes_block() -> None:
    _blocked(_payload(pb_1q=_pb_1q(release_continuity_review_hash="")), "PB_1Q_RELEASE_CONTINUITY_REVIEW_HASH_MISSING")


def test_missing_pb_1q_chronology_blocks() -> None:
    _blocked(_payload(pb_1q=_pb_1q(chronology_reference="")), "PB_1Q_CHRONOLOGY_REFERENCE_MISSING")


def test_missing_release_successor_authority_handoff_reference_blocks() -> None:
    _blocked(_payload(release_successor_authority_handoff=None), "RELEASE_SUCCESSOR_AUTHORITY_HANDOFF_REFERENCE_MISSING")


def test_missing_release_successor_authority_handoff_evidence_blocks() -> None:
    _blocked(_payload(release_successor_authority_handoff=_release_successor_authority_handoff(expected_release_successor_authority_handoff_evidence_hash=HASH)), "RELEASE_SUCCESSOR_AUTHORITY_HANDOFF_EVIDENCE_MISSING")
...
def test_duplicate_approval_metadata_blocks() -> None:
    _blocked(_payload(approval_chain=_approval(duplicate=True)), "DUPLICATE_SUCCESSOR_AUTHORITY_HANDOFF_METADATA")


def test_duplicate_successor_authority_handoff_metadata_blocks() -> None:
    _blocked(_payload(release_successor_authority_handoff=_release_successor_authority_handoff(duplicate=True)), "DUPLICATE_SUCCESSOR_AUTHORITY_HANDOFF_METADATA")
...
def test_stale_metadata_blocks() -> None:
    _blocked(_payload(pb_1q=_pb_1q(metadata_stale=True)), "STALE_METADATA")
...
def test_upstream_blocked_propagates() -> None:
    _blocked(_payload(pb_1q=_pb_1q(final_decision=BLOCKED)), "UPSTREAM_BLOCKED")


def test_hash_mismatch_blocks() -> None:
    _blocked(_payload(expected_release_successor_authority_handoff_hash=HASH), "RELEASE_SUCCESSOR_AUTHORITY_HANDOFF_PACKET_HASH_MISMATCH")
...
def test_malformed_metadata_blocks() -> None:
    _blocked(None, "MALFORMED_PB_1R_METADATA")


def test_deterministic_repeated_evaluation() -> None:
    payload = _payload()

    assert evaluate_pb_1r_release_successor_authority_handoff(payload).to_dict() == evaluate_pb_1r_release_successor_authority_handoff(payload).to_dict()


def test_canonical_decision_state_and_reason_order() -> None:
    result = evaluate_pb_1r_release_successor_authority_handoff(_payload(pb_1q=None, approval_chain=None))
...
def test_canonical_json_is_stable() -> None:
    decision = evaluate_pb_1r_release_successor_authority_handoff(_payload())

    assert canonical_pb_1r_release_successor_authority_handoff_json(decision) == canonical_pb_1r_release_successor_authority_handoff_json(decision)
...
def test_false_execution_flags_and_redacted_output() -> None:
    ...
    assert payload["release_successor_authority_handoff_hash"].startswith("sha256:")

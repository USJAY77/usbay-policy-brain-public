...
from runtime.computer_use.pb_1n_governance_release_authorization_readiness_handoff import (
    CAPABILITY_NAME,
    REQUIRED_VALIDATION_CHECKS,
    canonical_pb_1n_release_authorization_readiness_handoff_json,
    evaluate_pb_1n_release_authorization_readiness_handoff,
    expected_pb_1n_evidence_hash,
    expected_pb_1n_package_hash,
    expected_pb_1n_release_authorization_readiness_handoff_hash,
)
...
def _pb_1m(**overrides):
    record = {
        "final_decision": READY,
        "final_review_package_hash": HASH,
...
def _release_authorization_readiness(**overrides):
    record = {
        "release_authorization_readiness_reference": HASH,
        "release_authorization_readiness_evidence_hash": HASH_B,
        "expected_release_authorization_readiness_evidence_hash": HASH_B,
        "pb_1m_decision_hash": HASH_D,
        "pb_1m_package_hash": HASH_C,
...
def _evidence(**overrides):
    record = {
        "pb_1m_final_review_package_reference": HASH,
        "pb_1m_evidence_reference": HASH_B,
        "pb_1m_audit_reference": HASH,
        "release_authorization_readiness_evidence_reference": HASH_B,
...
def _rollback(**overrides):
    record = {
        ...
        "rollback_evidence_reference": HASH_C,
        "previous_release_authorization_readiness_handoff_hash": HASH_B,
...
def _replay(**overrides):
    record = {
        ...
        "duplicate_release_authorization_readiness_detected": False,
...
def _payload(**overrides):
    payload = {
        "pb_1m": _pb_1m(),
        "release_authorization_readiness": _release_authorization_readiness(),
    ...
    payload["expected_release_authorization_readiness_handoff_hash"] = expected_pb_1n_release_authorization_readiness_handoff_hash(payload)
    payload["expected_evidence_hash"] = expected_pb_1n_evidence_hash(payload)
    payload["expected_package_hash"] = expected_pb_1n_package_hash(payload)
    payload["rollback_references"]["current_release_authorization_readiness_handoff_hash"] = payload["expected_package_hash"]
...
def _blocked(payload, reason):
    result = evaluate_pb_1n_release_authorization_readiness_handoff(payload)
...
def test_valid_release_authorization_readiness_handoff_ready() -> None:
    result = evaluate_pb_1n_release_authorization_readiness_handoff(_payload())
...
def test_valid_lock_with_governed_restrictions() -> None:
    payload = _payload(
        pb_1m=_pb_1m(final_decision=READY_WITH_RESTRICTIONS),
    ...
    assert evaluate_pb_1n_release_authorization_readiness_handoff(payload).final_decision == READY_WITH_RESTRICTIONS


def test_missing_pb_1m_metadata_blocks() -> None:
    _blocked(_payload(pb_1m=None), "PB_1M_METADATA_MISSING")


def test_invalid_pb_1m_decision_blocks() -> None:
    _blocked(_payload(pb_1m=_pb_1m(final_decision="ALLOW")), "INVALID_PB_1M_DECISION")


def test_missing_pb_1m_hashes_block() -> None:
    _blocked(_payload(pb_1m=_pb_1m(final_review_package_hash="")), "PB_1M_FINAL_REVIEW_PACKAGE_HASH_MISSING")


def test_missing_release_authorization_readiness_reference_blocks() -> None:
    _blocked(_payload(release_authorization_readiness=None), "RELEASE_AUTHORIZATION_READINESS_REFERENCE_MISSING")


def test_missing_release_authorization_readiness_evidence_blocks() -> None:
    _blocked(_payload(release_authorization_readiness=_release_authorization_readiness(expected_release_authorization_readiness_evidence_hash=HASH)), "RELEASE_AUTHORIZATION_READINESS_EVIDENCE_MISSING")
...
def test_duplicate_approval_metadata_blocks() -> None:
    _blocked(_payload(approval_chain=_approval(duplicate=True)), "DUPLICATE_RELEASE_AUTHORIZATION_READINESS_METADATA")


def test_duplicate_release_authorization_readiness_metadata_blocks() -> None:
    _blocked(_payload(release_authorization_readiness=_release_authorization_readiness(duplicate=True)), "DUPLICATE_RELEASE_AUTHORIZATION_READINESS_METADATA")


def test_duplicate_package_metadata_blocks() -> None:
    _blocked(_payload(replay_protection=_replay(duplicate_package_detected=True)), "DUPLICATE_RELEASE_AUTHORIZATION_READINESS_METADATA")
...
def test_stale_metadata_blocks() -> None:
    _blocked(_payload(pb_1m=_pb_1m(metadata_stale=True)), "STALE_METADATA")
...
def test_upstream_blocked_propagates() -> None:
    _blocked(_payload(pb_1m=_pb_1m(final_decision=BLOCKED)), "UPSTREAM_BLOCKED")


def test_hash_mismatch_blocks() -> None:
    _blocked(_payload(expected_release_authorization_readiness_handoff_hash=HASH), "RELEASE_AUTHORIZATION_READINESS_HANDOFF_HASH_MISMATCH")
...
def test_malformed_metadata_blocks() -> None:
    _blocked(None, "MALFORMED_PB_1N_METADATA")


def test_deterministic_repeated_evaluation() -> None:
    payload = _payload()

    assert evaluate_pb_1n_release_authorization_readiness_handoff(payload).to_dict() == evaluate_pb_1n_release_authorization_readiness_handoff(payload).to_dict()


def test_canonical_decision_state_and_reason_order() -> None:
    result = evaluate_pb_1n_release_authorization_readiness_handoff(_payload(pb_1m=None, approval_chain=None))
...
def test_canonical_json_is_stable() -> None:
    decision = evaluate_pb_1n_release_authorization_readiness_handoff(_payload())

    assert canonical_pb_1n_release_authorization_readiness_handoff_json(decision) == canonical_pb_1n_release_authorization_readiness_handoff_json(decision)
...
def test_false_execution_flags_and_redacted_output() -> None:
    ...
    assert payload["release_authorization_readiness_handoff_hash"].startswith("sha256:")

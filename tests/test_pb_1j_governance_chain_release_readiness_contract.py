from __future__ import annotations

import json

from runtime.computer_use.pb_1j_governance_chain_release_readiness_contract import (
    BLOCKED,
    CAPABILITY_NAME,
    READY,
    READY_WITH_RESTRICTIONS,
    REQUIRED_VALIDATION_CHECKS,
    canonical_pb_1j_json,
    evaluate_pb_1j_release_readiness_contract,
    expected_pb_1j_contract_hash,
    expected_pb_1j_evidence_hash,
    expected_pb_1j_package_hash,
)


HASH = "sha256:" + ("a" * 64)
HASH_B = "sha256:" + ("b" * 64)
HASH_C = "sha256:" + ("c" * 64)
HASH_D = "sha256:" + ("d" * 64)


def _pb_1i(**overrides):
    record = {
        "final_decision": READY,
        "chain_hash": HASH,
        "evidence_hash": HASH_B,
        "package_hash": HASH_C,
        "decision_hash": HASH_D,
        "audit_hash": HASH,
        "policy_version": "policy-v1",
        "metadata_stale": False,
    }
    record.update(overrides)
    return record


def _release_intent(**overrides):
    record = {
        "release_intent_reference": HASH,
        "capability": CAPABILITY_NAME,
        "policy_version": "policy-v1",
        "metadata_stale": False,
    }
    record.update(overrides)
    return record


def _approval(**overrides):
    record = {
        "status": "VALID",
        "approval_reference": HASH_D,
        "capability": CAPABILITY_NAME,
        "pb_1i_decision_hash": HASH_D,
        "policy_version": "policy-v1",
        "release_intent_reference": HASH,
        "regression_evidence_reference": HASH_B,
        "rollback_evidence_reference": HASH_C,
        "chronology_reference": HASH,
        "expired": False,
        "metadata_stale": False,
    }
    record.update(overrides)
    return record


def _audit(**overrides):
    record = {
        "pb_1i_audit_reference": HASH,
        "pb_1j_validation_audit_reference": HASH_B,
        "previous_audit_hash": HASH_C,
        "current_audit_hash": HASH_D,
        "expected_current_audit_hash": HASH_D,
        "correlation_reference": HASH,
        "policy_version": "policy-v1",
        "human_approval_reference": HASH_D,
        "chronology_marker": 1,
        "duplicate": False,
        "metadata_stale": False,
    }
    record.update(overrides)
    return record


def _evidence(**overrides):
    record = {
        "pb_1i_evidence_reference": HASH_B,
        "regression_test_evidence_reference": HASH_B,
        "governance_regression_evidence_reference": HASH,
        "approval_evidence_reference": HASH_D,
        "audit_chain_evidence_reference": HASH,
        "rollback_evidence_reference": HASH_C,
        "boundary_verification_evidence_reference": HASH,
        "sensitive_data_scan_evidence_reference": HASH,
        "execution_surface_scan_evidence_reference": HASH,
        "current_evidence_hash": HASH_D,
        "expected_current_evidence_hash": HASH_D,
        "metadata_stale": False,
    }
    record.update(overrides)
    return record


def _regression(**overrides):
    record = {"status": "PASS", "regression_evidence_reference": HASH_B, "metadata_stale": False}
    record.update(overrides)
    return record


def _rollback(**overrides):
    record = {"status": "VERIFIED", "rollback_evidence_reference": HASH_C, "metadata_stale": False}
    record.update(overrides)
    return record


def _replay(**overrides):
    record = {
        "nonce_reference": HASH,
        "timestamp_reference": HASH_B,
        "previous_package_hash": HASH_C,
        "current_package_hash": "",
        "replay_window_seconds": 300,
        "duplicate_package_detected": False,
        "chronology_marker": 2,
        "expired": False,
        "metadata_stale": False,
    }
    record.update(overrides)
    return record


def _validation(**overrides):
    checks = {check: {"status": "PASS", "evidence_reference": HASH} for check in REQUIRED_VALIDATION_CHECKS}
    checks.update(overrides)
    return checks


def _safety(**overrides):
    flags = {
        "execution_allowed": False,
        "provider_execution": False,
        "production_activation": False,
        "deployment_authorized": False,
        "runtime_mutation": False,
        "policy_mutation": False,
    }
    flags.update(overrides)
    return flags


def _payload(**overrides):
    payload = {
        "pb_1i": _pb_1i(),
        "release_intent": _release_intent(),
        "approval_chain": _approval(),
        "audit_chain": _audit(),
        "evidence_chain": _evidence(),
        "regression_evidence": _regression(),
        "rollback_evidence": _rollback(),
        "replay_protection": _replay(),
        "validation_metadata": _validation(),
        "safety_flags": _safety(),
        "restriction_metadata": [],
    }
    payload["expected_contract_hash"] = expected_pb_1j_contract_hash(payload)
    payload["expected_evidence_hash"] = expected_pb_1j_evidence_hash(payload)
    payload["expected_package_hash"] = expected_pb_1j_package_hash(payload)
    payload["replay_protection"]["current_package_hash"] = payload["expected_package_hash"]
    payload.update(overrides)
    return payload


def _blocked(payload, reason):
    result = evaluate_pb_1j_release_readiness_contract(payload)

    assert result.final_decision == BLOCKED
    assert reason in result.reason_codes
    assert result.execution_allowed is False
    assert result.provider_execution is False
    assert result.production_activation is False
    assert result.deployment_authorized is False
    assert result.runtime_mutation is False
    assert result.policy_mutation is False
    assert result.evidence_hash.startswith("sha256:")
    return result


def test_valid_complete_chain_ready() -> None:
    result = evaluate_pb_1j_release_readiness_contract(_payload())

    assert result.final_decision == READY
    assert result.reason_codes == ()


def test_ready_with_restrictions() -> None:
    payload = _payload(
        pb_1i=_pb_1i(final_decision=READY_WITH_RESTRICTIONS),
        restriction_metadata=[{"restriction_reference": HASH}],
    )
    payload["expected_contract_hash"] = expected_pb_1j_contract_hash(payload)
    payload["expected_evidence_hash"] = expected_pb_1j_evidence_hash(payload)
    payload["expected_package_hash"] = expected_pb_1j_package_hash(payload)
    payload["replay_protection"]["current_package_hash"] = payload["expected_package_hash"]

    assert evaluate_pb_1j_release_readiness_contract(payload).final_decision == READY_WITH_RESTRICTIONS


def test_missing_pb_1i_metadata_blocks() -> None:
    _blocked(_payload(pb_1i=None), "PB_1I_METADATA_MISSING")


def test_invalid_pb_1i_decision_blocks() -> None:
    _blocked(_payload(pb_1i=_pb_1i(final_decision="ALLOW")), "INVALID_PB_1I_DECISION")


def test_missing_pb_1i_hashes_block() -> None:
    _blocked(_payload(pb_1i=_pb_1i(chain_hash="")), "PB_1I_CHAIN_HASH_MISSING")


def test_missing_release_intent_blocks() -> None:
    _blocked(_payload(release_intent=None), "RELEASE_INTENT_MISSING")


def test_missing_approval_blocks() -> None:
    _blocked(_payload(approval_chain=None), "APPROVAL_REFERENCE_MISSING")


def test_invalid_approval_blocks() -> None:
    _blocked(_payload(approval_chain=_approval(status="EXPIRED")), "APPROVAL_INVALID")


def test_missing_audit_chain_blocks() -> None:
    _blocked(_payload(audit_chain=None), "AUDIT_CHAIN_MISSING")


def test_mismatched_audit_hash_blocks() -> None:
    _blocked(_payload(audit_chain=_audit(expected_current_audit_hash=HASH)), "AUDIT_HASH_MISMATCH")


def test_missing_evidence_chain_blocks() -> None:
    _blocked(_payload(evidence_chain=None), "EVIDENCE_CHAIN_MISSING")


def test_mismatched_evidence_hash_blocks() -> None:
    _blocked(_payload(evidence_chain=_evidence(expected_current_evidence_hash=HASH)), "EVIDENCE_HASH_MISMATCH")


def test_missing_regression_evidence_blocks() -> None:
    _blocked(_payload(regression_evidence=None), "REGRESSION_EVIDENCE_MISSING")


def test_missing_rollback_evidence_blocks() -> None:
    _blocked(_payload(rollback_evidence=None), "ROLLBACK_EVIDENCE_MISSING")


def test_missing_replay_metadata_blocks() -> None:
    _blocked(_payload(replay_protection=None), "REPLAY_METADATA_MISSING")


def test_replayed_input_blocks() -> None:
    _blocked(_payload(replay_protection=_replay(duplicate_package_detected=True)), "DUPLICATE_PACKAGE_METADATA")


def test_invalid_nonce_blocks() -> None:
    _blocked(_payload(replay_protection=_replay(nonce_reference="")), "REPLAY_METADATA_MISSING")


def test_expired_timestamp_window_blocks() -> None:
    _blocked(_payload(replay_protection=_replay(expired=True)), "STALE_METADATA")


def test_chronology_mismatch_blocks() -> None:
    _blocked(_payload(replay_protection=_replay(chronology_marker=0)), "CHRONOLOGY_MISMATCH")


def test_package_hash_mismatch_blocks() -> None:
    _blocked(_payload(expected_package_hash=HASH), "PACKAGE_HASH_MISMATCH")


def test_replay_package_hash_mismatch_blocks() -> None:
    _blocked(_payload(replay_protection=_replay(current_package_hash=HASH)), "REPLAY_PACKAGE_HASH_MISMATCH")


def test_stale_metadata_blocks() -> None:
    _blocked(_payload(pb_1i=_pb_1i(metadata_stale=True)), "STALE_METADATA")


def test_unsupported_metadata_blocks() -> None:
    _blocked(_payload(unknown_field=HASH), "UNSUPPORTED_CAPABILITY_METADATA")


def test_upstream_blocked_propagates() -> None:
    _blocked(_payload(pb_1i=_pb_1i(final_decision=BLOCKED)), "UPSTREAM_BLOCKED")


def test_direct_execution_rejected() -> None:
    _blocked(_payload(direct_execution_requested=True), "EXECUTION_SURFACE_REJECTED")


def test_provider_execution_rejected() -> None:
    _blocked(_payload(safety_flags=_safety(provider_execution=True)), "EXECUTION_FLAG_NOT_FALSE")


def test_deployment_rejected() -> None:
    _blocked(_payload(safety_flags=_safety(deployment_authorized=True)), "EXECUTION_FLAG_NOT_FALSE")


def test_production_activation_rejected() -> None:
    _blocked(_payload(safety_flags=_safety(production_activation=True)), "EXECUTION_FLAG_NOT_FALSE")


def test_runtime_mutation_rejected() -> None:
    _blocked(_payload(safety_flags=_safety(runtime_mutation=True)), "EXECUTION_FLAG_NOT_FALSE")


def test_network_subprocess_rejected() -> None:
    _blocked(_payload(network_access=True), "EXECUTION_SURFACE_REJECTED")


def test_sensitive_data_rejected_without_leakage() -> None:
    result = _blocked(_payload(raw_payload="synthetic-sensitive"), "SENSITIVE_DATA_REJECTED")

    assert "synthetic-sensitive" not in json.dumps(result.to_dict(), sort_keys=True)


def test_credential_literal_rejected_without_leakage() -> None:
    synthetic_marker = "g" + "hp_" + ("x" * 36)
    result = _blocked(_payload(reference=synthetic_marker), "CREDENTIAL_LITERAL_REJECTED")

    assert synthetic_marker not in json.dumps(result.to_dict(), sort_keys=True)


def test_deterministic_repeated_evaluation() -> None:
    payload = _payload()

    assert evaluate_pb_1j_release_readiness_contract(payload).to_dict() == evaluate_pb_1j_release_readiness_contract(payload).to_dict()


def test_canonical_decision_state_and_reason_order() -> None:
    result = evaluate_pb_1j_release_readiness_contract(_payload(pb_1i=None, release_intent=None))

    assert result.final_decision in {READY, READY_WITH_RESTRICTIONS, BLOCKED}
    assert result.reason_codes == tuple(sorted(result.reason_codes))


def test_canonical_json_is_stable() -> None:
    decision = evaluate_pb_1j_release_readiness_contract(_payload())

    assert canonical_pb_1j_json(decision) == canonical_pb_1j_json(decision)


def test_redacted_evidence_output() -> None:
    result = evaluate_pb_1j_release_readiness_contract(_payload())
    payload = result.to_dict()

    assert payload["contract_hash"].startswith("sha256:")
    assert payload["evidence_hash"].startswith("sha256:")
    assert payload["package_hash"].startswith("sha256:")
    assert payload["execution_allowed"] is False
    assert payload["provider_execution"] is False
    assert payload["production_activation"] is False

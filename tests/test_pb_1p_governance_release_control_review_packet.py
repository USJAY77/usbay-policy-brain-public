...
from runtime.computer_use.pb_1p_governance_release_control_review_packet import (
    CAPABILITY_NAME,
    REQUIRED_VALIDATION_CHECKS,
    canonical_pb_1p_release_control_review_packet_json,
    evaluate_pb_1p_release_control_review_packet,
    expected_pb_1p_evidence_hash,
    expected_pb_1p_package_hash,
    expected_pb_1p_release_control_review_packet_hash,
)
...
def _pb_1o(**overrides):
    record = {
        "final_decision": READY,
        "release_authority_review_dossier_hash": HASH,
...
def _release_control_review(**overrides):
    record = {
        "release_control_review_reference": HASH,
        "release_control_review_evidence_hash": HASH_B,
        "expected_release_control_review_evidence_hash": HASH_B,
        "pb_1o_decision_hash": HASH_D,
        "pb_1o_package_hash": HASH_C,
...
def _evidence(**overrides):
    record = {
        "pb_1o_release_authority_review_dossier_reference": HASH,
        "pb_1o_evidence_reference": HASH_B,
        "pb_1o_audit_reference": HASH,
        "release_control_review_evidence_reference": HASH_B,
...
def _rollback(**overrides):
    record = {
        ...
        "rollback_evidence_reference": HASH_C,
        "previous_release_control_review_packet_hash": HASH_B,
...
def _replay(**overrides):
    record = {
        ...
        "duplicate_control_review_detected": False,
...
def _payload(**overrides):
    payload = {
        "pb_1o": _pb_1o(),
        "release_control_review": _release_control_review(),
    ...
    payload["expected_release_control_review_packet_hash"] = expected_pb_1p_release_control_review_packet_hash(payload)
    payload["expected_evidence_hash"] = expected_pb_1p_evidence_hash(payload)
    payload["expected_package_hash"] = expected_pb_1p_package_hash(payload)
    payload["rollback_references"]["current_release_control_review_packet_hash"] = payload["expected_package_hash"]
...
def _blocked(payload, reason):
    result = evaluate_pb_1p_release_control_review_packet(payload)
...
def test_valid_release_control_review_packet_ready() -> None:
    result = evaluate_pb_1p_release_control_review_packet(_payload())
...
def test_valid_packet_with_governed_restrictions() -> None:
    payload = _payload(
        pb_1o=_pb_1o(final_decision=READY_WITH_RESTRICTIONS),
    ...
    assert evaluate_pb_1p_release_control_review_packet(payload).final_decision == READY_WITH_RESTRICTIONS


def test_missing_pb_1o_metadata_blocks() -> None:
    _blocked(_payload(pb_1o=None), "PB_1O_METADATA_MISSING")


def test_invalid_pb_1o_decision_blocks() -> None:
    _blocked(_payload(pb_1o=_pb_1o(final_decision="ALLOW")), "INVALID_PB_1O_DECISION")


def test_missing_pb_1o_hashes_block() -> None:
    _blocked(_payload(pb_1o=_pb_1o(release_authority_review_dossier_hash="")), "PB_1O_RELEASE_AUTHORITY_REVIEW_DOSSIER_HASH_MISSING")


def test_missing_pb_1o_chronology_blocks() -> None:
    _blocked(_payload(pb_1o=_pb_1o(chronology_reference="")), "PB_1O_CHRONOLOGY_REFERENCE_MISSING")


def test_missing_release_control_review_reference_blocks() -> None:
    _blocked(_payload(release_control_review=None), "RELEASE_CONTROL_REVIEW_REFERENCE_MISSING")


def test_missing_release_control_review_evidence_blocks() -> None:
    _blocked(_payload(release_control_review=_release_control_review(expected_release_control_review_evidence_hash=HASH)), "RELEASE_CONTROL_REVIEW_EVIDENCE_MISSING")
...
def test_duplicate_approval_metadata_blocks() -> None:
    _blocked(_payload(approval_chain=_approval(duplicate=True)), "DUPLICATE_CONTROL_REVIEW_METADATA")


def test_duplicate_control_review_metadata_blocks() -> None:
    _blocked(_payload(release_control_review=_release_control_review(duplicate=True)), "DUPLICATE_CONTROL_REVIEW_METADATA")
...
def test_stale_metadata_blocks() -> None:
    _blocked(_payload(pb_1o=_pb_1o(metadata_stale=True)), "STALE_METADATA")
...
def test_upstream_blocked_propagates() -> None:
    _blocked(_payload(pb_1o=_pb_1o(final_decision=BLOCKED)), "UPSTREAM_BLOCKED")


def test_hash_mismatch_blocks() -> None:
    _blocked(_payload(expected_release_control_review_packet_hash=HASH), "RELEASE_CONTROL_REVIEW_PACKET_HASH_MISMATCH")
...
def test_malformed_metadata_blocks() -> None:
    _blocked(None, "MALFORMED_PB_1P_METADATA")


def test_deterministic_repeated_evaluation() -> None:
    payload = _payload()

    assert evaluate_pb_1p_release_control_review_packet(payload).to_dict() == evaluate_pb_1p_release_control_review_packet(payload).to_dict()


def test_canonical_decision_state_and_reason_order() -> None:
    result = evaluate_pb_1p_release_control_review_packet(_payload(pb_1o=None, approval_chain=None))
...
def test_canonical_json_is_stable() -> None:
    decision = evaluate_pb_1p_release_control_review_packet(_payload())

    assert canonical_pb_1p_release_control_review_packet_json(decision) == canonical_pb_1p_release_control_review_packet_json(decision)
...
def test_false_execution_flags_and_redacted_output() -> None:
    ...
    assert payload["release_control_review_packet_hash"].startswith("sha256:")

...
## Executive Report is 12 (NOT 11) numbered sections — user-confirmed order, Risk Summary stays
The board report's canonical, user-confirmed section order is: 1 Executive summary, 2 Governance challenge, 3 Recommended license, 4 Governance risk summary, 5 Governance gaps, 6 Pilot scope, 7 Governance objectives, 8 Human oversight requirements, 9 Evidence requirements, 10 Success criteria, 11 Pilot timeline, 12 Recommended next step. "Governance objectives" (#rpt-objectives) renders the wizard milestones: `setList('#rpt-objectives', s ? wizApi.selected(wizApi.milestoneDefaults(s), wizApi.checks.milestones) : null)` — same source as the wizard plan's "Governance objectives" block.
**Why:** the user explicitly chose to KEEP "Governance risk summary" alongside their listed sections (it gives board-level context for why the license/scope were chosen), so the report is 12 sections, not 11. Single source of truth chain: Assessment → License → Pilot Wizard → Executive Governance Report.
**How to apply:** when editing report sections keep heading numeric prefixes (`N · …` in HTML ~2379-2434) AND the `// N · …` build comments (~5084-5155) in lockstep 1..12; do not drop Risk Summary. No PDF/export/download — preview-only modal by design.

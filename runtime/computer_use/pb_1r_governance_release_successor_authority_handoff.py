...
CAPABILITY_NAME = "PB-1R Governance Release Successor Authority Handoff"
ALLOWED_DECISIONS = frozenset({READY, READY_WITH_RESTRICTIONS, BLOCKED})
ALLOWED_TOP_LEVEL_FIELDS = frozenset(
    {
        "pb_1q",
        "release_successor_authority_handoff",
        "approval_chain",
        "approval_evidence",
        "audit_chain",
        "evidence_chain",
        ...
        "expected_release_successor_authority_handoff_hash",
...
REDACTED_HASH_FIELDS = frozenset(
    {
        ...
        "current_release_successor_authority_handoff_hash",
        "raw_payload",
        "secret",
        "token",
    }
...
class PB1RReleaseSuccessorAuthorityHandoffDecision:
    final_decision: str
    reason_codes: tuple[str, ...]
    release_successor_authority_handoff_hash: str
    evidence_hash: str
    package_hash: str
    ...
    def to_dict(self) -> dict[str, Any]:
        payload = {
            ...
            "release_successor_authority_handoff_hash": self.release_successor_authority_handoff_hash,
...
def evaluate_pb_1r_release_successor_authority_handoff(
    metadata: Mapping[str, Any] | None,
) -> PB1RReleaseSuccessorAuthorityHandoffDecision:
    try:
        return _evaluate_pb_1r_release_successor_authority_handoff(metadata)
    except Exception:
        return _blocked(["INTERNAL_ERROR"])


def expected_pb_1r_release_successor_authority_handoff_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1R_METADATA"})
        return _release_successor_authority_handoff_hash(metadata)
    except Exception:
        return sha256_reference({"blocked": "INTERNAL_ERROR"})


def expected_pb_1r_evidence_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1R_METADATA"})
...
def expected_pb_1r_package_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1R_METADATA"})
...
def canonical_pb_1r_release_successor_authority_handoff_json(
    decision: PB1RReleaseSuccessorAuthorityHandoffDecision,
) -> str:
    return canonical_json(decision.to_dict())


def _evaluate_pb_1r_release_successor_authority_handoff(
    metadata: Mapping[str, Any] | None,
) -> PB1RReleaseSuccessorAuthorityHandoffDecision:
    if not isinstance(metadata, Mapping):
        return _blocked(["MALFORMED_PB_1R_METADATA"])
    ...
    reasons = [
        *_pb_1q_reasons(metadata.get("pb_1q")),
        *_release_successor_authority_handoff_reasons(metadata.get("release_successor_authority_handoff"), metadata),
        ...
        *_dependency_reasons(metadata.get("dependency_references"), metadata),
        *_successor_reasons(metadata.get("successor_references")),
    ...
    return _decision(
        final_decision,
        [],
        release_successor_authority_handoff_hash=_release_successor_authority_handoff_hash(metadata),
...
def _pb_1q_reasons(pb_1q: Any) -> tuple[str, ...]:
    if not isinstance(pb_1q, Mapping):
        return ("PB_1Q_METADATA_MISSING",)
    reasons: list[str] = []
    decision = pb_1q.get("final_decision")
    if decision not in ALLOWED_DECISIONS:
        reasons.append("INVALID_PB_1Q_DECISION")
    if decision == BLOCKED:
        reasons.append("UPSTREAM_BLOCKED")
    for field, reason in (
        ("release_continuity_review_hash", "PB_1Q_RELEASE_CONTINUITY_REVIEW_HASH_MISSING"),
        ("evidence_hash", "PB_1Q_EVIDENCE_HASH_MISSING"),
        ("package_hash", "PB_1Q_PACKAGE_HASH_MISSING"),
        ("decision_hash", "PB_1Q_DECISION_HASH_MISSING"),
        ("audit_hash", "PB_1Q_AUDIT_HASH_MISSING"),
        ("chronology_reference", "PB_1Q_CHRONOLOGY_REFERENCE_MISSING"),
    ):
        if not is_sha256_reference(pb_1q.get(field)):
            reasons.append(reason)
    if pb_1q.get("metadata_stale") is True:
        reasons.append("STALE_METADATA")
    return tuple(reasons)


def _release_successor_authority_handoff_reasons(candidate: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    if not isinstance(candidate, Mapping):
        return ("RELEASE_SUCCESSOR_AUTHORITY_HANDOFF_REFERENCE_MISSING",)
    reasons: list[str] = []
    for field in (
        "release_successor_authority_handoff_reference",
        "release_successor_authority_handoff_evidence_hash",
        "expected_release_successor_authority_handoff_evidence_hash",
        "pb_1q_decision_hash",
        "pb_1q_package_hash",
        "policy_reference",
        "tenant_reference",
        ...
        if not is_sha256_reference(candidate.get(field)):
            reasons.append("RELEASE_SUCCESSOR_AUTHORITY_HANDOFF_REFERENCE_MISSING")
    if candidate.get("release_successor_authority_handoff_evidence_hash") != candidate.get("expected_release_successor_authority_handoff_evidence_hash"):
        reasons.append("RELEASE_SUCCESSOR_AUTHORITY_HANDOFF_EVIDENCE_MISSING")
    if candidate.get("pb_1q_decision_hash") != _mapping_value(metadata.get("pb_1q"), "decision_hash"):
        reasons.append("RELEASE_SUCCESSOR_AUTHORITY_HANDOFF_EVIDENCE_MISSING")
    if candidate.get("pb_1q_package_hash") != _mapping_value(metadata.get("pb_1q"), "package_hash"):
        reasons.append("RELEASE_SUCCESSOR_AUTHORITY_HANDOFF_EVIDENCE_MISSING")
    ...
    if candidate.get("correlation_reference") != _mapping_value(metadata.get("correlation_reference"), "correlation_reference"):
        reasons.append("CORRELATION_REFERENCE_MISMATCH")
    if candidate.get("duplicate") is True:
        reasons.append("DUPLICATE_SUCCESSOR_AUTHORITY_HANDOFF_METADATA")
...
def _policy_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if not isinstance(policy.get("policy_version"), str) or not policy.get("policy_version"):
        reasons.append("POLICY_VERSION_MISSING")
    if isinstance(pb_1q, Mapping) and policy.get("policy_version") != pb_1q.get("policy_version"):
        reasons.append("POLICY_VERSION_MISMATCH")
...
def _correlation_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if correlation.get("expected_correlation_reference") != correlation.get("correlation_reference"):
        reasons.append("CORRELATION_REFERENCE_MISMATCH")
    return tuple(reasons)
...
def _approval_reasons(approval: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if approval.get("capability") != CAPABILITY_NAME:
        reasons.append("APPROVAL_INVALID")
    if approval.get("pb_1q_decision_hash") != _mapping_value(metadata.get("pb_1q"), "decision_hash"):
        reasons.append("APPROVAL_INVALID")
    if approval.get("pb_1q_package_hash") != _mapping_value(metadata.get("pb_1q"), "package_hash"):
        reasons.append("APPROVAL_INVALID")
    if approval.get("release_successor_authority_handoff_reference") != _mapping_value(metadata.get("release_successor_authority_handoff"), "release_successor_authority_handoff_reference"):
        reasons.append("APPROVAL_INVALID")
    if approval.get("release_successor_authority_handoff_evidence_hash") != _mapping_value(metadata.get("release_successor_authority_handoff"), "release_successor_authority_handoff_evidence_hash"):
        reasons.append("APPROVAL_INVALID")
...
def _audit_reasons(audit: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    for field in (
        "pb_1q_audit_hash",
        "pb_1r_validation_audit_hash",
        "previous_audit_hash",
        "current_audit_hash",
        "expected_current_audit_hash",
        "approval_audit_reference",
        "release_successor_authority_handoff_audit_reference",
        "correlation_reference",
        "policy_reference",
        "tenant_reference",
    ):
    ...
    if audit.get("pb_1q_audit_hash") != _mapping_value(metadata.get("pb_1q"), "audit_hash"):
        reasons.append("AUDIT_HASH_MISMATCH")
    if audit.get("approval_audit_reference") != _mapping_value(metadata.get("approval_chain"), "approval_reference"):
        reasons.append("AUDIT_HASH_MISMATCH")
    if audit.get("release_successor_authority_handoff_audit_reference") != _mapping_value(metadata.get("release_successor_authority_handoff"), "release_successor_authority_handoff_reference"):
        reasons.append("AUDIT_HASH_MISMATCH")
...
def _evidence_reasons(evidence: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    reasons: list[str] = []
    for field in (
        "pb_1q_release_continuity_review_reference",
        "pb_1q_evidence_reference",
        "pb_1q_audit_reference",
        "release_successor_authority_handoff_evidence_reference",
        "approval_evidence_reference",
        "audit_chain_evidence_reference",
        "evidence_chain_reference",
        "dependency_evidence_reference",
        "replay_evidence_reference",
        "rollback_evidence_reference",
        "validation_evidence_reference",
        "successor_handoff_evidence_reference",
    ...
    if evidence.get("pb_1q_release_continuity_review_reference") != _mapping_value(metadata.get("pb_1q"), "release_continuity_review_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
    if evidence.get("pb_1q_evidence_reference") != _mapping_value(metadata.get("pb_1q"), "evidence_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
    if evidence.get("pb_1q_audit_reference") != _mapping_value(metadata.get("pb_1q"), "audit_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
    if evidence.get("release_successor_authority_handoff_evidence_reference") != _mapping_value(metadata.get("release_successor_authority_handoff"), "release_successor_authority_handoff_evidence_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
...
def _dependency_reasons(dependencies: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    if not isinstance(dependencies, Mapping):
        return ("DEPENDENCY_REFERENCE_MISSING",)
    reasons: list[str] = []
    for field in (
        "predecessor_dependency_reference",
        "successor_dependency_reference",
        "governance_dependency_readiness_reference",
        "audit_dependency_readiness_reference",
        "evidence_dependency_readiness_reference",
        "rollback_dependency_readiness_reference",
        "approval_dependency_readiness_reference",
    ):
        if not is_sha256_reference(dependencies.get(field)):
            reasons.append("DEPENDENCY_REFERENCE_MISSING")
    if dependencies.get("predecessor_dependency_reference") != _mapping_value(metadata.get("pb_1q"), "release_continuity_review_hash"):
        reasons.append("DEPENDENCY_REFERENCE_MISMATCH")
    if dependencies.get("approval_dependency_readiness_reference") != _mapping_value(metadata.get("approval_chain"), "approval_reference"):
        reasons.append("DEPENDENCY_REFERENCE_MISMATCH")
    if dependencies.get("duplicate") is True:
        reasons.append("DUPLICATE_DEPENDENCY_METADATA")
    if dependencies.get("metadata_stale") is True:
        reasons.append("STALE_METADATA")
    return tuple(reasons)


def _successor_reasons(successor: Any) -> tuple[str, ...]:
    if not isinstance(successor, Mapping):
        return ("SUCCESSOR_REFERENCE_MISSING",)
    reasons: list[str] = []
    for field in (
        "successor_capability_reference",
        "successor_handoff_reference",
        "successor_evidence_reference",
        "successor_audit_reference",
        "successor_chronology_reference",
        "successor_dependency_reference",
    ):
        if not is_sha256_reference(successor.get(field)):
            reasons.append("SUCCESSOR_REFERENCE_MISSING")
    if successor.get("duplicate") is True:
        reasons.append("DUPLICATE_SUCCESSOR_METADATA")
    if successor.get("metadata_stale") is True:
        reasons.append("STALE_METADATA")
    return tuple(reasons)
...
def _rollback_reasons(rollback: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    for field in (
        ...
        "rollback_evidence_reference",
        "previous_release_successor_authority_handoff_hash",
        "current_release_successor_authority_handoff_hash",
    ...
    if rollback.get("status") != "VERIFIED":
        reasons.append("ROLLBACK_EVIDENCE_MISSING")
    if rollback.get("current_release_successor_authority_handoff_hash") != metadata.get("expected_package_hash"):
        reasons.append("PACKAGE_HASH_MISMATCH")
    if rollback.get("duplicate") is True:
        reasons.append("DUPLICATE_PACKET_METADATA")
...
def _replay_reasons(replay: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if replay.get("duplicate_approval_detected") is not False:
        reasons.append("DUPLICATE_SUCCESSOR_AUTHORITY_HANDOFF_METADATA")
    if replay.get("duplicate_successor_authority_handoff_detected") is not False:
        reasons.append("DUPLICATE_SUCCESSOR_AUTHORITY_HANDOFF_METADATA")
    if replay.get("duplicate_packet_detected") is not False:
        reasons.append("DUPLICATE_PACKET_METADATA")
...
def _restriction_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    pb_1q = metadata.get("pb_1q")
    pb_1q_restricted = isinstance(pb_1q, Mapping) and pb_1q.get("final_decision") == READY_WITH_RESTRICTIONS
    restrictions = metadata.get("restriction_metadata", ())
    if pb_1q_restricted and not _has_governed_restrictions(metadata):
...
def _hash_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    for field, expected, mismatch in (
        (
            "expected_release_successor_authority_handoff_hash",
            _release_successor_authority_handoff_hash(metadata),
            "RELEASE_SUCCESSOR_AUTHORITY_HANDOFF_PACKET_HASH_MISMATCH",
        ),
...
def _release_successor_authority_handoff_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference({"schema": "usbay.pb_1r.release_successor_authority_handoff.v1", "metadata": _redacted_metadata(metadata)})


def _evidence_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference(
        {
            "schema": "usbay.pb_1r.release_successor_authority_handoff.evidence.v1",
            "release_successor_authority_handoff_hash": _release_successor_authority_handoff_hash(metadata),
...
def _package_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference(
        {
            "schema": "usbay.pb_1r.release_successor_authority_handoff.package.v1",
            "evidence_hash": _evidence_hash(metadata),
            "release_successor_authority_handoff_hash": _release_successor_authority_handoff_hash(metadata),
        }
    )
...
def _redacted_metadata(metadata: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "pb_1q": _redacted_mapping(metadata.get("pb_1q")),
        "release_successor_authority_handoff": _redacted_mapping(metadata.get("release_successor_authority_handoff")),
        ...
        "audit_chain": _redacted_mapping(metadata.get("audit_chain")),
        "evidence_chain": _redacted_mapping(metadata.get("evidence_chain")),
        "dependency_references": _redacted_mapping(metadata.get("dependency_references")),
        "successor_references": _redacted_mapping(metadata.get("successor_references")),
        "rollback_references": _redacted_mapping(metadata.get("rollback_references")),
...
def _decision(
    ...
    *,
    release_successor_authority_handoff_hash: str | None = None,
    evidence_hash: str | None = None,
    package_hash: str | None = None,
    ) -> PB1RReleaseSuccessorAuthorityHandoffDecision:
    normalized_reasons = tuple(sorted(dict.fromkeys(reason_codes)))
    resolved_packet_hash = release_successor_authority_handoff_hash or sha256_reference(
        {"final_decision": final_decision, "reason_codes": normalized_reasons}
    )
    resolved_evidence_hash = evidence_hash or sha256_reference(
        {"final_decision": final_decision, "reason_codes": normalized_reasons, "release_successor_authority_handoff_hash": resolved_packet_hash}
    )
    resolved_package_hash = package_hash or sha256_reference(
        {
            ...
            "release_successor_authority_handoff_hash": resolved_packet_hash,
        }
    )
    return PB1RReleaseSuccessorAuthorityHandoffDecision(
        final_decision=final_decision,
        reason_codes=normalized_reasons,
        release_successor_authority_handoff_hash=resolved_packet_hash,
        evidence_hash=resolved_evidence_hash,
...
def _blocked(reason_codes: Sequence[str]) -> PB1RReleaseSuccessorAuthorityHandoffDecision:

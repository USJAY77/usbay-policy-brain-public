...
CAPABILITY_NAME = "PB-1M Governance Final Release Review Package"
...
ALLOWED_TOP_LEVEL_FIELDS = frozenset(
    {
        ...
        "expected_final_review_package_hash",
...
REDACTED_HASH_FIELDS = frozenset(
    {
        ...
        "current_final_review_package_hash",
        "raw_payload",
        "secret",
        "token",
    }
...
class PB1MFinalReleaseReviewPackageDecision:
    final_decision: str
    reason_codes: tuple[str, ...]
    final_review_package_hash: str
    evidence_hash: str
    package_hash: str
    ...
    def to_dict(self) -> dict[str, Any]:
        payload = {
            ...
            "final_review_package_hash": self.final_review_package_hash,
...
def evaluate_pb_1m_final_release_review_package(
    metadata: Mapping[str, Any] | None,
) -> PB1MFinalReleaseReviewPackageDecision:
    try:
        return _evaluate_pb_1m_final_release_review_package(metadata)
    except Exception:
        return _blocked(["INTERNAL_ERROR"])


def expected_pb_1m_final_review_package_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1M_METADATA"})
        return _final_review_package_hash(metadata)
    except Exception:
        return sha256_reference({"blocked": "INTERNAL_ERROR"})


def expected_pb_1m_evidence_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1M_METADATA"})
...
def expected_pb_1m_package_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1M_METADATA"})
...
def canonical_pb_1m_json(decision: PB1MFinalReleaseReviewPackageDecision) -> str:
    return canonical_json(decision.to_dict())


def _evaluate_pb_1m_final_release_review_package(
    metadata: Mapping[str, Any] | None,
) -> PB1MFinalReleaseReviewPackageDecision:
    if not isinstance(metadata, Mapping):
        return _blocked(["MALFORMED_PB_1M_METADATA"])
    ...
    reasons = [
        *_pb_1l_reasons(metadata.get("pb_1l")),
        *_final_review_reasons(metadata.get("final_review"), metadata),
    ...
    return _decision(
        final_decision,
        [],
        final_review_package_hash=_final_review_package_hash(metadata),
...
def _pb_1l_reasons(pb_1l: Any) -> tuple[str, ...]:
    if not isinstance(pb_1l, Mapping):
        return ("PB_1L_METADATA_MISSING",)
    reasons: list[str] = []
    decision = pb_1l.get("final_decision")
    if decision not in ALLOWED_DECISIONS:
        reasons.append("INVALID_PB_1L_DECISION")
    if decision == BLOCKED:
        reasons.append("UPSTREAM_BLOCKED")
    for field, reason in (
        ("release_candidate_lock_hash", "PB_1L_RELEASE_CANDIDATE_LOCK_HASH_MISSING"),
        ("evidence_hash", "PB_1L_EVIDENCE_HASH_MISSING"),
        ("package_hash", "PB_1L_PACKAGE_HASH_MISSING"),
        ("decision_hash", "PB_1L_DECISION_HASH_MISSING"),
        ("audit_hash", "PB_1L_AUDIT_HASH_MISSING"),
    ):
        if not is_sha256_reference(pb_1l.get(field)):
            reasons.append(reason)
    if pb_1l.get("metadata_stale") is True:
        reasons.append("STALE_METADATA")
    return tuple(reasons)


def _final_review_reasons(candidate: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    if not isinstance(candidate, Mapping):
        return ("FINAL_REVIEW_REFERENCE_MISSING",)
    reasons: list[str] = []
    for field in (
        "final_review_reference",
        "final_review_evidence_hash",
        "expected_final_review_evidence_hash",
        "pb_1l_decision_hash",
        "pb_1l_package_hash",
        "policy_reference",
        "tenant_reference",
        ...
        if not is_sha256_reference(candidate.get(field)):
            reasons.append("FINAL_REVIEW_REFERENCE_MISSING")
    if candidate.get("final_review_evidence_hash") != candidate.get("expected_final_review_evidence_hash"):
        reasons.append("FINAL_REVIEW_EVIDENCE_MISSING")
    if candidate.get("pb_1l_decision_hash") != _mapping_value(metadata.get("pb_1l"), "decision_hash"):
        reasons.append("FINAL_REVIEW_EVIDENCE_MISSING")
    if candidate.get("pb_1l_package_hash") != _mapping_value(metadata.get("pb_1l"), "package_hash"):
        reasons.append("FINAL_REVIEW_EVIDENCE_MISSING")
    ...
    if candidate.get("duplicate") is True:
        reasons.append("DUPLICATE_FINAL_REVIEW_METADATA")
...
def _policy_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if not isinstance(policy.get("policy_version"), str) or not policy.get("policy_version"):
        reasons.append("POLICY_VERSION_MISSING")
    if isinstance(pb_1l, Mapping) and policy.get("policy_version") != pb_1l.get("policy_version"):
        reasons.append("POLICY_VERSION_MISMATCH")
...
def _approval_reasons(approval: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if approval.get("capability") != CAPABILITY_NAME:
        reasons.append("APPROVAL_INVALID")
    if approval.get("pb_1l_decision_hash") != _mapping_value(metadata.get("pb_1l"), "decision_hash"):
        reasons.append("APPROVAL_INVALID")
    if approval.get("pb_1l_package_hash") != _mapping_value(metadata.get("pb_1l"), "package_hash"):
        reasons.append("APPROVAL_INVALID")
    if approval.get("final_review_reference") != _mapping_value(metadata.get("final_review"), "final_review_reference"):
        reasons.append("APPROVAL_INVALID")
    if approval.get("final_review_evidence_hash") != _mapping_value(metadata.get("final_review"), "final_review_evidence_hash"):
        reasons.append("APPROVAL_INVALID")
...
def _audit_reasons(audit: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    for field in (
        "pb_1l_audit_hash",
        "pb_1m_validation_audit_hash",
        "previous_audit_hash",
        "current_audit_hash",
        "expected_current_audit_hash",
        "approval_audit_reference",
        "final_review_audit_reference",
        "correlation_reference",
        "policy_reference",
        "tenant_reference",
    ):
    ...
    if audit.get("pb_1l_audit_hash") != _mapping_value(metadata.get("pb_1l"), "audit_hash"):
        reasons.append("AUDIT_HASH_MISMATCH")
    if audit.get("approval_audit_reference") != _mapping_value(metadata.get("approval_chain"), "approval_reference"):
        reasons.append("AUDIT_HASH_MISMATCH")
    if audit.get("final_review_audit_reference") != _mapping_value(metadata.get("final_review"), "final_review_reference"):
        reasons.append("AUDIT_HASH_MISMATCH")
...
def _evidence_reasons(evidence: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    reasons: list[str] = []
    for field in (
        "pb_1l_release_candidate_lock_reference",
        "pb_1l_evidence_reference",
        "pb_1l_audit_reference",
        "final_review_evidence_reference",
    ...
    if evidence.get("pb_1l_release_candidate_lock_reference") != _mapping_value(metadata.get("pb_1l"), "release_candidate_lock_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
    if evidence.get("pb_1l_evidence_reference") != _mapping_value(metadata.get("pb_1l"), "evidence_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
    if evidence.get("pb_1l_audit_reference") != _mapping_value(metadata.get("pb_1l"), "audit_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
    if evidence.get("final_review_evidence_reference") != _mapping_value(metadata.get("final_review"), "final_review_evidence_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
...
def _rollback_reasons(rollback: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    for field in (
        ...
        "rollback_evidence_reference",
        "previous_final_review_package_hash",
        "current_final_review_package_hash",
    ...
    if rollback.get("status") != "VERIFIED":
        reasons.append("ROLLBACK_EVIDENCE_MISSING")
    if rollback.get("current_final_review_package_hash") != metadata.get("expected_package_hash"):
...
def _replay_reasons(replay: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if replay.get("duplicate_approval_detected") is not False:
        reasons.append("DUPLICATE_FINAL_REVIEW_METADATA")
    if replay.get("duplicate_final_review_detected") is not False:
        reasons.append("DUPLICATE_FINAL_REVIEW_METADATA")
...
def _restriction_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    pb_1l = metadata.get("pb_1l")
    pb_1l_restricted = isinstance(pb_1l, Mapping) and pb_1l.get("final_decision") == READY_WITH_RESTRICTIONS
    restrictions = metadata.get("restriction_metadata", ())
    if pb_1l_restricted and not _has_governed_restrictions(metadata):
...
def _hash_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    for field, expected, mismatch in (
        (
            "expected_final_review_package_hash",
            _final_review_package_hash(metadata),
...
def _final_review_package_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference({"schema": "usbay.pb_1m.final_release_review_package.v1", "metadata": _redacted_metadata(metadata)})


def _evidence_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference(
        {
            "schema": "usbay.pb_1m.final_release_review_package.evidence.v1",
            "final_review_package_hash": _final_review_package_hash(metadata),
...
def _package_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference(
        {
            "schema": "usbay.pb_1m.final_release_review_package.package.v1",
            "evidence_hash": _evidence_hash(metadata),
            "final_review_package_hash": _final_review_package_hash(metadata),
        }
    )
...
def _redacted_metadata(metadata: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "pb_1l": _redacted_mapping(metadata.get("pb_1l")),
        "final_review": _redacted_mapping(metadata.get("final_review")),
...
def _decision(
    ...
    *,
    final_review_package_hash: str | None = None,
    evidence_hash: str | None = None,
    package_hash: str | None = None,
) -> PB1MFinalReleaseReviewPackageDecision:
    normalized_reasons = tuple(sorted(dict.fromkeys(reason_codes)))
    resolved_lock_hash = final_review_package_hash or sha256_reference(
        {"final_decision": final_decision, "reason_codes": normalized_reasons}
    )
    resolved_evidence_hash = evidence_hash or sha256_reference(
        {"final_decision": final_decision, "reason_codes": normalized_reasons, "final_review_package_hash": resolved_lock_hash}
    )
    resolved_package_hash = package_hash or sha256_reference(
        {
            ...
            "final_review_package_hash": resolved_lock_hash,
        }
    )
    return PB1MFinalReleaseReviewPackageDecision(
        final_decision=final_decision,
        reason_codes=normalized_reasons,
        final_review_package_hash=resolved_lock_hash,
        evidence_hash=resolved_evidence_hash,
...
def _blocked(reason_codes: Sequence[str]) -> PB1MFinalReleaseReviewPackageDecision:

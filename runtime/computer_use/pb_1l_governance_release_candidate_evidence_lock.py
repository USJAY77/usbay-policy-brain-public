...
CAPABILITY_NAME = "PB-1L Governance Release Candidate Evidence Lock"
...
ALLOWED_TOP_LEVEL_FIELDS = frozenset(
    {
        ...
        "expected_release_candidate_lock_hash",
...
class PB1LReleaseCandidateEvidenceLockDecision:
    final_decision: str
    reason_codes: tuple[str, ...]
    release_candidate_lock_hash: str
    evidence_hash: str
    package_hash: str
    ...
    def to_dict(self) -> dict[str, Any]:
        payload = {
            ...
            "release_candidate_lock_hash": self.release_candidate_lock_hash,
...
def evaluate_pb_1l_release_candidate_evidence_lock(
    metadata: Mapping[str, Any] | None,
) -> PB1LReleaseCandidateEvidenceLockDecision:
    try:
        return _evaluate_pb_1l_release_candidate_evidence_lock(metadata)
    except Exception:
        return _blocked(["INTERNAL_ERROR"])


def expected_pb_1l_release_candidate_lock_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1L_METADATA"})
        return _release_candidate_lock_hash(metadata)
    except Exception:
        return sha256_reference({"blocked": "INTERNAL_ERROR"})


def expected_pb_1l_evidence_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1L_METADATA"})
...
def expected_pb_1l_package_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1L_METADATA"})
...
def canonical_pb_1l_json(decision: PB1LReleaseCandidateEvidenceLockDecision) -> str:
    return canonical_json(decision.to_dict())


def _evaluate_pb_1l_release_candidate_evidence_lock(
    metadata: Mapping[str, Any] | None,
) -> PB1LReleaseCandidateEvidenceLockDecision:
    if not isinstance(metadata, Mapping):
        return _blocked(["MALFORMED_PB_1L_METADATA"])
    ...
    reasons = [
        *_pb_1k_reasons(metadata.get("pb_1k")),
        *_release_candidate_reasons(metadata.get("release_candidate"), metadata),
    ...
    return _decision(
        final_decision,
        [],
        release_candidate_lock_hash=_release_candidate_lock_hash(metadata),
...
def _pb_1k_reasons(pb_1k: Any) -> tuple[str, ...]:
    if not isinstance(pb_1k, Mapping):
        return ("PB_1K_METADATA_MISSING",)
    reasons: list[str] = []
    decision = pb_1k.get("final_decision")
    if decision not in ALLOWED_DECISIONS:
        reasons.append("INVALID_PB_1K_DECISION")
    if decision == BLOCKED:
        reasons.append("UPSTREAM_BLOCKED")
    for field, reason in (
        ("approval_closure_hash", "PB_1K_APPROVAL_CLOSURE_HASH_MISSING"),
        ("evidence_hash", "PB_1K_EVIDENCE_HASH_MISSING"),
        ("package_hash", "PB_1K_PACKAGE_HASH_MISSING"),
        ("decision_hash", "PB_1K_DECISION_HASH_MISSING"),
        ("audit_hash", "PB_1K_AUDIT_HASH_MISSING"),
    ):
        if not is_sha256_reference(pb_1k.get(field)):
            reasons.append(reason)
    if pb_1k.get("metadata_stale") is True:
        reasons.append("STALE_METADATA")
    return tuple(reasons)


def _release_candidate_reasons(candidate: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    if not isinstance(candidate, Mapping):
        return ("RELEASE_CANDIDATE_REFERENCE_MISSING",)
    reasons: list[str] = []
    for field in (
        "release_candidate_reference",
        "release_candidate_evidence_hash",
        "expected_release_candidate_evidence_hash",
        "pb_1k_decision_hash",
        "pb_1k_package_hash",
        "policy_reference",
        "tenant_reference",
        ...
        if not is_sha256_reference(candidate.get(field)):
            reasons.append("RELEASE_CANDIDATE_REFERENCE_MISSING")
    if candidate.get("release_candidate_evidence_hash") != candidate.get("expected_release_candidate_evidence_hash"):
        reasons.append("RELEASE_CANDIDATE_EVIDENCE_MISSING")
    if candidate.get("pb_1k_decision_hash") != _mapping_value(metadata.get("pb_1k"), "decision_hash"):
        reasons.append("RELEASE_CANDIDATE_EVIDENCE_MISSING")
    if candidate.get("pb_1k_package_hash") != _mapping_value(metadata.get("pb_1k"), "package_hash"):
        reasons.append("RELEASE_CANDIDATE_EVIDENCE_MISSING")
    ...
    if candidate.get("duplicate") is True:
        reasons.append("DUPLICATE_RELEASE_CANDIDATE_METADATA")
...
def _policy_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if not isinstance(policy.get("policy_version"), str) or not policy.get("policy_version"):
        reasons.append("POLICY_VERSION_MISSING")
    if isinstance(pb_1k, Mapping) and policy.get("policy_version") != pb_1k.get("policy_version"):
        reasons.append("POLICY_VERSION_MISMATCH")
...
def _approval_reasons(approval: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if approval.get("capability") != CAPABILITY_NAME:
        reasons.append("APPROVAL_INVALID")
    if approval.get("pb_1k_decision_hash") != _mapping_value(metadata.get("pb_1k"), "decision_hash"):
        reasons.append("APPROVAL_INVALID")
    if approval.get("pb_1k_package_hash") != _mapping_value(metadata.get("pb_1k"), "package_hash"):
        reasons.append("APPROVAL_INVALID")
    if approval.get("release_candidate_reference") != _mapping_value(metadata.get("release_candidate"), "release_candidate_reference"):
        reasons.append("APPROVAL_INVALID")
    if approval.get("release_candidate_evidence_hash") != _mapping_value(metadata.get("release_candidate"), "release_candidate_evidence_hash"):
        reasons.append("APPROVAL_INVALID")
...
def _audit_reasons(audit: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    for field in (
        "pb_1k_audit_hash",
        "pb_1l_validation_audit_hash",
        "previous_audit_hash",
        "current_audit_hash",
        "expected_current_audit_hash",
        "approval_audit_reference",
        "release_candidate_audit_reference",
        "correlation_reference",
        "policy_reference",
        "tenant_reference",
    ):
    ...
    if audit.get("pb_1k_audit_hash") != _mapping_value(metadata.get("pb_1k"), "audit_hash"):
        reasons.append("AUDIT_HASH_MISMATCH")
    if audit.get("approval_audit_reference") != _mapping_value(metadata.get("approval_chain"), "approval_reference"):
        reasons.append("AUDIT_HASH_MISMATCH")
    if audit.get("release_candidate_audit_reference") != _mapping_value(metadata.get("release_candidate"), "release_candidate_reference"):
        reasons.append("AUDIT_HASH_MISMATCH")
...
def _evidence_reasons(evidence: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    reasons: list[str] = []
    for field in (
        "pb_1k_approval_closure_reference",
        "pb_1k_evidence_reference",
        "pb_1k_audit_reference",
        "release_candidate_evidence_reference",
    ...
    if evidence.get("pb_1k_approval_closure_reference") != _mapping_value(metadata.get("pb_1k"), "approval_closure_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
    if evidence.get("pb_1k_evidence_reference") != _mapping_value(metadata.get("pb_1k"), "evidence_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
    if evidence.get("pb_1k_audit_reference") != _mapping_value(metadata.get("pb_1k"), "audit_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
    if evidence.get("release_candidate_evidence_reference") != _mapping_value(metadata.get("release_candidate"), "release_candidate_evidence_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
...
def _rollback_reasons(rollback: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    for field in (
        ...
        "rollback_evidence_reference",
        "previous_release_candidate_package_hash",
        "current_release_candidate_package_hash",
    ...
    if rollback.get("status") != "VERIFIED":
        reasons.append("ROLLBACK_EVIDENCE_MISSING")
    if rollback.get("current_release_candidate_package_hash") != metadata.get("expected_package_hash"):
...
def _replay_reasons(replay: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if replay.get("duplicate_approval_detected") is not False:
        reasons.append("DUPLICATE_RELEASE_CANDIDATE_METADATA")
    if replay.get("duplicate_release_candidate_detected") is not False:
        reasons.append("DUPLICATE_RELEASE_CANDIDATE_METADATA")
...
def _restriction_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    pb_1k = metadata.get("pb_1k")
    pb_1k_restricted = isinstance(pb_1k, Mapping) and pb_1k.get("final_decision") == READY_WITH_RESTRICTIONS
    restrictions = metadata.get("restriction_metadata", ())
    if pb_1k_restricted and not _has_governed_restrictions(metadata):
...
def _hash_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    for field, expected, mismatch in (
        (
            "expected_release_candidate_lock_hash",
            _release_candidate_lock_hash(metadata),
...
def _release_candidate_lock_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference({"schema": "usbay.pb_1l.release_candidate_evidence_lock.v1", "metadata": _redacted_metadata(metadata)})


def _evidence_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference(
        {
            "schema": "usbay.pb_1l.release_candidate_evidence_lock.evidence.v1",
            "release_candidate_lock_hash": _release_candidate_lock_hash(metadata),
...
def _package_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference(
        {
            "schema": "usbay.pb_1l.release_candidate_evidence_lock.package.v1",
            "evidence_hash": _evidence_hash(metadata),
            "release_candidate_lock_hash": _release_candidate_lock_hash(metadata),
        }
    )
...
def _redacted_metadata(metadata: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "pb_1k": _redacted_mapping(metadata.get("pb_1k")),
        "release_candidate": _redacted_mapping(metadata.get("release_candidate")),
...
def _decision(
    ...
    *,
    release_candidate_lock_hash: str | None = None,
    evidence_hash: str | None = None,
    package_hash: str | None = None,
) -> PB1LReleaseCandidateEvidenceLockDecision:
    normalized_reasons = tuple(sorted(dict.fromkeys(reason_codes)))
    resolved_lock_hash = release_candidate_lock_hash or sha256_reference(
        {"final_decision": final_decision, "reason_codes": normalized_reasons}
    )
    resolved_evidence_hash = evidence_hash or sha256_reference(
        {"final_decision": final_decision, "reason_codes": normalized_reasons, "release_candidate_lock_hash": resolved_lock_hash}
    )
    resolved_package_hash = package_hash or sha256_reference(
        {
            ...
            "release_candidate_lock_hash": resolved_lock_hash,
        }
    )
    return PB1LReleaseCandidateEvidenceLockDecision(
        final_decision=final_decision,
        reason_codes=normalized_reasons,
        release_candidate_lock_hash=resolved_lock_hash,
        evidence_hash=resolved_evidence_hash,
...
def _blocked(reason_codes: Sequence[str]) -> PB1LReleaseCandidateEvidenceLockDecision:

...
CAPABILITY_NAME = "PB-1K Governance Release Approval Evidence Closure"
...
ALLOWED_TOP_LEVEL_FIELDS = frozenset(
    {
        ...
        "expected_approval_closure_hash",
...
class PB1KApprovalEvidenceClosureDecision:
    final_decision: str
    reason_codes: tuple[str, ...]
    approval_closure_hash: str
    evidence_hash: str
    package_hash: str
    ...
    def to_dict(self) -> dict[str, Any]:
        payload = {
            ...
            "approval_closure_hash": self.approval_closure_hash,
...
def evaluate_pb_1k_approval_evidence_closure(metadata: Mapping[str, Any] | None) -> PB1KApprovalEvidenceClosureDecision:
    try:
        return _evaluate_pb_1k_approval_evidence_closure(metadata)
    except Exception:
        return _blocked(["INTERNAL_ERROR"])


def expected_pb_1k_approval_closure_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1K_METADATA"})
        return _approval_closure_hash(metadata)
    except Exception:
        return sha256_reference({"blocked": "INTERNAL_ERROR"})


def expected_pb_1k_evidence_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1K_METADATA"})
...
def expected_pb_1k_package_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1K_METADATA"})
...
def canonical_pb_1k_json(decision: PB1KApprovalEvidenceClosureDecision) -> str:
    return canonical_json(decision.to_dict())


def _evaluate_pb_1k_approval_evidence_closure(metadata: Mapping[str, Any] | None) -> PB1KApprovalEvidenceClosureDecision:
    if not isinstance(metadata, Mapping):
        return _blocked(["MALFORMED_PB_1K_METADATA"])
    ...
    reasons = [
        *_pb_1j_reasons(metadata.get("pb_1j")),
    ...
    return _decision(
        final_decision,
        [],
        approval_closure_hash=_approval_closure_hash(metadata),
...
def _pb_1j_reasons(pb_1j: Any) -> tuple[str, ...]:
    if not isinstance(pb_1j, Mapping):
        return ("PB_1J_METADATA_MISSING",)
    reasons: list[str] = []
    decision = pb_1j.get("final_decision")
    if decision not in ALLOWED_DECISIONS:
        reasons.append("INVALID_PB_1J_DECISION")
    if decision == BLOCKED:
        reasons.append("UPSTREAM_BLOCKED")
    for field, reason in (
        ("contract_hash", "PB_1J_CONTRACT_HASH_MISSING"),
        ("evidence_hash", "PB_1J_EVIDENCE_HASH_MISSING"),
        ("package_hash", "PB_1J_PACKAGE_HASH_MISSING"),
        ("decision_hash", "PB_1J_DECISION_HASH_MISSING"),
        ("audit_hash", "PB_1J_AUDIT_HASH_MISSING"),
    ):
        if not is_sha256_reference(pb_1j.get(field)):
            reasons.append(reason)
    if pb_1j.get("metadata_stale") is True:
...
def _policy_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if not isinstance(policy.get("policy_version"), str) or not policy.get("policy_version"):
        reasons.append("POLICY_VERSION_MISSING")
    if isinstance(pb_1j, Mapping) and policy.get("policy_version") != pb_1j.get("policy_version"):
...
def _approval_reasons(approval: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if approval.get("duplicate") is True:
        reasons.append("DUPLICATE_APPROVAL_METADATA")
    for field in (
        "approval_reference",
    ...
    if approval.get("capability") != CAPABILITY_NAME:
        reasons.append("APPROVAL_INVALID")
    if approval.get("pb_1j_decision_hash") != _mapping_value(metadata.get("pb_1j"), "decision_hash"):
        reasons.append("APPROVAL_INVALID")
    if approval.get("pb_1j_package_hash") != _mapping_value(metadata.get("pb_1j"), "package_hash"):
        reasons.append("APPROVAL_INVALID")
...
def _audit_reasons(audit: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if audit.get("pb_1j_audit_hash") != _mapping_value(metadata.get("pb_1j"), "audit_hash"):
        reasons.append("AUDIT_HASH_MISMATCH")
...
def _evidence_reasons(evidence: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    reasons: list[str] = []
    for field in (
        "pb_1j_evidence_reference",
        "pb_1j_audit_reference",
    ...
    if evidence.get("pb_1j_evidence_reference") != _mapping_value(metadata.get("pb_1j"), "evidence_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
    if evidence.get("pb_1j_audit_reference") != _mapping_value(metadata.get("pb_1j"), "audit_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
...
def _restriction_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    pb_1j = metadata.get("pb_1j")
    pb_1j_restricted = isinstance(pb_1j, Mapping) and pb_1j.get("final_decision") == READY_WITH_RESTRICTIONS
    restrictions = metadata.get("restriction_metadata", ())
    if pb_1j_restricted and not _has_governed_restrictions(metadata):
...
def _hash_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    for field, expected, mismatch in (
        ("expected_approval_closure_hash", _approval_closure_hash(metadata), "APPROVAL_CLOSURE_HASH_MISMATCH"),
...
def _approval_closure_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference({"schema": "usbay.pb_1k.approval_evidence_closure.v1", "metadata": _redacted_metadata(metadata)})


def _evidence_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference(
        {
            "schema": "usbay.pb_1k.approval_evidence_closure.evidence.v1",
            "approval_closure_hash": _approval_closure_hash(metadata),
...
def _package_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference(
        {
            "schema": "usbay.pb_1k.approval_evidence_closure.package.v1",
            "approval_closure_hash": _approval_closure_hash(metadata),
            "evidence_hash": _evidence_hash(metadata),
        }
    )
...
def _redacted_metadata(metadata: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "pb_1j": _redacted_mapping(metadata.get("pb_1j")),
...
def _redacted_mapping(value: Any) -> dict[str, Any]:
    ...
    return {
        ...
        if key not in {"approval_contents", "raw_payload", "secret", "token", "credential", "current_package_hash"}
    }
...
def _decision(
    ...
    *,
    approval_closure_hash: str | None = None,
    evidence_hash: str | None = None,
    package_hash: str | None = None,
) -> PB1KApprovalEvidenceClosureDecision:
    normalized_reasons = tuple(sorted(dict.fromkeys(reason_codes)))
    resolved_closure_hash = approval_closure_hash or sha256_reference({"final_decision": final_decision, "reason_codes": normalized_reasons})
    resolved_evidence_hash = evidence_hash or sha256_reference(
        {"approval_closure_hash": resolved_closure_hash, "final_decision": final_decision, "reason_codes": normalized_reasons}
    )
    resolved_package_hash = package_hash or sha256_reference(
        {
            "approval_closure_hash": resolved_closure_hash,
            "evidence_hash": resolved_evidence_hash,
            "final_decision": final_decision,
            "reason_codes": normalized_reasons,
        }
    )
    return PB1KApprovalEvidenceClosureDecision(
        final_decision=final_decision,
        reason_codes=normalized_reasons,
        approval_closure_hash=resolved_closure_hash,
        evidence_hash=resolved_evidence_hash,
...
def _blocked(reason_codes: Sequence[str]) -> PB1KApprovalEvidenceClosureDecision:

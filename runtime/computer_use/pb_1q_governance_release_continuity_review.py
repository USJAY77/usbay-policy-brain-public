...
ALLOWED_TOP_LEVEL_FIELDS = frozenset(
    {
        ...
        "expected_release_continuity_review_hash",
...
class PB1QReleaseContinuityReviewDecision:
    final_decision: str
    reason_codes: tuple[str, ...]
    release_continuity_review_hash: str
    evidence_hash: str
    package_hash: str
    ...
    def to_dict(self) -> dict[str, Any]:
        payload = {
            ...
            "release_continuity_review_hash": self.release_continuity_review_hash,
...
def evaluate_pb_1q_release_continuity_review(
    metadata: Mapping[str, Any] | None,
) -> PB1QReleaseContinuityReviewDecision:
    try:
        return _evaluate_pb_1q_release_continuity_review(metadata)
    except Exception:
        return _blocked(["INTERNAL_ERROR"])


def expected_pb_1q_release_continuity_review_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1Q_METADATA"})
        return _release_continuity_review_hash(metadata)
    except Exception:
        return sha256_reference({"blocked": "INTERNAL_ERROR"})


def expected_pb_1q_evidence_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1Q_METADATA"})
...
def expected_pb_1q_package_hash(metadata: Mapping[str, Any] | None) -> str:
    try:
        if not isinstance(metadata, Mapping):
            return sha256_reference({"blocked": "MALFORMED_PB_1Q_METADATA"})
...
def canonical_pb_1q_release_continuity_review_json(decision: PB1QReleaseContinuityReviewDecision) -> str:
    return canonical_json(decision.to_dict())


def _evaluate_pb_1q_release_continuity_review(
    metadata: Mapping[str, Any] | None,
) -> PB1QReleaseContinuityReviewDecision:
    if not isinstance(metadata, Mapping):
        return _blocked(["MALFORMED_PB_1Q_METADATA"])
    ...
    reasons = [
        *_pb_1p_reasons(metadata.get("pb_1p")),
        *_release_continuity_review_reasons(metadata.get("release_continuity_review"), metadata),
    ...
    return _decision(
        final_decision,
        [],
        release_continuity_review_hash=_release_continuity_review_hash(metadata),
...
def _pb_1p_reasons(pb_1p: Any) -> tuple[str, ...]:
    if not isinstance(pb_1p, Mapping):
        return ("PB_1P_METADATA_MISSING",)
    reasons: list[str] = []
    decision = pb_1p.get("final_decision")
    if decision not in ALLOWED_DECISIONS:
        reasons.append("INVALID_PB_1P_DECISION")
    if decision == BLOCKED:
        reasons.append("UPSTREAM_BLOCKED")
    for field, reason in (
        ("release_control_review_packet_hash", "PB_1P_RELEASE_CONTROL_REVIEW_PACKET_HASH_MISSING"),
        ("evidence_hash", "PB_1P_EVIDENCE_HASH_MISSING"),
        ("package_hash", "PB_1P_PACKAGE_HASH_MISSING"),
        ("decision_hash", "PB_1P_DECISION_HASH_MISSING"),
        ("audit_hash", "PB_1P_AUDIT_HASH_MISSING"),
        ("chronology_reference", "PB_1P_CHRONOLOGY_REFERENCE_MISSING"),
    ):
        if not is_sha256_reference(pb_1p.get(field)):
            reasons.append(reason)
    if pb_1p.get("metadata_stale") is True:
        reasons.append("STALE_METADATA")
    return tuple(reasons)


def _release_continuity_review_reasons(candidate: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    if not isinstance(candidate, Mapping):
        return ("RELEASE_CONTINUITY_REVIEW_REFERENCE_MISSING",)
    reasons: list[str] = []
    for field in (
        "release_continuity_review_reference",
        "release_continuity_review_evidence_hash",
        "expected_release_continuity_review_evidence_hash",
        "pb_1p_decision_hash",
        "pb_1p_package_hash",
        "policy_reference",
        "tenant_reference",
        ...
        if not is_sha256_reference(candidate.get(field)):
            reasons.append("RELEASE_CONTINUITY_REVIEW_REFERENCE_MISSING")
    if candidate.get("release_continuity_review_evidence_hash") != candidate.get("expected_release_continuity_review_evidence_hash"):
        reasons.append("RELEASE_CONTINUITY_REVIEW_EVIDENCE_MISSING")
    if candidate.get("pb_1p_decision_hash") != _mapping_value(metadata.get("pb_1p"), "decision_hash"):
        reasons.append("RELEASE_CONTINUITY_REVIEW_EVIDENCE_MISSING")
    if candidate.get("pb_1p_package_hash") != _mapping_value(metadata.get("pb_1p"), "package_hash"):
        reasons.append("RELEASE_CONTINUITY_REVIEW_EVIDENCE_MISSING")
    ...
    if candidate.get("duplicate") is True:
        reasons.append("DUPLICATE_CONTINUITY_REVIEW_METADATA")
...
def _policy_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if not isinstance(policy.get("policy_version"), str) or not policy.get("policy_version"):
        reasons.append("POLICY_VERSION_MISSING")
    if isinstance(pb_1p, Mapping) and policy.get("policy_version") != pb_1p.get("policy_version"):
        reasons.append("POLICY_VERSION_MISMATCH")
...
def _approval_reasons(approval: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if approval.get("capability") != CAPABILITY_NAME:
        reasons.append("APPROVAL_INVALID")
    if approval.get("pb_1p_decision_hash") != _mapping_value(metadata.get("pb_1p"), "decision_hash"):
        reasons.append("APPROVAL_INVALID")
    if approval.get("pb_1p_package_hash") != _mapping_value(metadata.get("pb_1p"), "package_hash"):
        reasons.append("APPROVAL_INVALID")
    if approval.get("release_continuity_review_reference") != _mapping_value(metadata.get("release_continuity_review"), "release_continuity_review_reference"):
        reasons.append("APPROVAL_INVALID")
    if approval.get("release_continuity_review_evidence_hash") != _mapping_value(metadata.get("release_continuity_review"), "release_continuity_review_evidence_hash"):
        reasons.append("APPROVAL_INVALID")
...
def _audit_reasons(audit: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    for field in (
        "pb_1p_audit_hash",
        "pb_1q_validation_audit_hash",
        "previous_audit_hash",
        "current_audit_hash",
        "expected_current_audit_hash",
        "approval_audit_reference",
        "release_continuity_review_audit_reference",
        "correlation_reference",
        "policy_reference",
        "tenant_reference",
    ):
    ...
    if audit.get("pb_1p_audit_hash") != _mapping_value(metadata.get("pb_1p"), "audit_hash"):
        reasons.append("AUDIT_HASH_MISMATCH")
    if audit.get("approval_audit_reference") != _mapping_value(metadata.get("approval_chain"), "approval_reference"):
        reasons.append("AUDIT_HASH_MISMATCH")
    if audit.get("release_continuity_review_audit_reference") != _mapping_value(metadata.get("release_continuity_review"), "release_continuity_review_reference"):
        reasons.append("AUDIT_HASH_MISMATCH")
...
def _evidence_reasons(evidence: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    reasons: list[str] = []
    for field in (
        "pb_1p_release_control_review_packet_reference",
        "pb_1p_evidence_reference",
        "pb_1p_audit_reference",
        "release_continuity_review_evidence_reference",
    ...
    if evidence.get("pb_1p_release_control_review_packet_reference") != _mapping_value(metadata.get("pb_1p"), "release_control_review_packet_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
    if evidence.get("pb_1p_evidence_reference") != _mapping_value(metadata.get("pb_1p"), "evidence_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
    if evidence.get("pb_1p_audit_reference") != _mapping_value(metadata.get("pb_1p"), "audit_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
    if evidence.get("release_continuity_review_evidence_reference") != _mapping_value(metadata.get("release_continuity_review"), "release_continuity_review_evidence_hash"):
        reasons.append("EVIDENCE_HASH_MISMATCH")
...
def _dependency_reasons(dependencies: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if dependencies.get("predecessor_dependency_reference") != _mapping_value(metadata.get("pb_1p"), "release_control_review_packet_hash"):
...
def _rollback_reasons(rollback: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    for field in (
        ...
        "rollback_evidence_reference",
        "previous_release_continuity_review_hash",
        "current_release_continuity_review_hash",
    ...
    if rollback.get("status") != "VERIFIED":
        reasons.append("ROLLBACK_EVIDENCE_MISSING")
    if rollback.get("current_release_continuity_review_hash") != metadata.get("expected_package_hash"):
...
def _replay_reasons(replay: Any, metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if replay.get("duplicate_approval_detected") is not False:
        reasons.append("DUPLICATE_CONTINUITY_REVIEW_METADATA")
    if replay.get("duplicate_continuity_review_detected") is not False:
        reasons.append("DUPLICATE_CONTINUITY_REVIEW_METADATA")
...
def _restriction_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    pb_1p = metadata.get("pb_1p")
    pb_1p_restricted = isinstance(pb_1p, Mapping) and pb_1p.get("final_decision") == READY_WITH_RESTRICTIONS
    restrictions = metadata.get("restriction_metadata", ())
    if pb_1p_restricted and not _has_governed_restrictions(metadata):
...
def _hash_reasons(metadata: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    for field, expected, mismatch in (
        (
            "expected_release_continuity_review_hash",
            _release_continuity_review_hash(metadata),
            "RELEASE_CONTINUITY_REVIEW_PACKET_HASH_MISMATCH",
        ),
...
def _release_continuity_review_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference({"schema": "usbay.pb_1q.release_continuity_review.v1", "metadata": _redacted_metadata(metadata)})


def _evidence_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference(
        {
            "schema": "usbay.pb_1q.release_continuity_review.evidence.v1",
            "release_continuity_review_hash": _release_continuity_review_hash(metadata),
...
def _package_hash(metadata: Mapping[str, Any]) -> str:
    return sha256_reference(
        {
            "schema": "usbay.pb_1q.release_continuity_review.package.v1",
            "evidence_hash": _evidence_hash(metadata),
            "release_continuity_review_hash": _release_continuity_review_hash(metadata),
        }
    )
...
def _redacted_metadata(metadata: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "pb_1p": _redacted_mapping(metadata.get("pb_1p")),
        "release_continuity_review": _redacted_mapping(metadata.get("release_continuity_review")),
...
def _decision(
    ...
    *,
    release_continuity_review_hash: str | None = None,
    evidence_hash: str | None = None,
    package_hash: str | None = None,
) -> PB1QReleaseContinuityReviewDecision:
    normalized_reasons = tuple(sorted(dict.fromkeys(reason_codes)))
    resolved_packet_hash = release_continuity_review_hash or sha256_reference(
        {"final_decision": final_decision, "reason_codes": normalized_reasons}
    )
    resolved_evidence_hash = evidence_hash or sha256_reference(
        {"final_decision": final_decision, "reason_codes": normalized_reasons, "release_continuity_review_hash": resolved_packet_hash}
    )
    resolved_package_hash = package_hash or sha256_reference(
        {
            ...
            "release_continuity_review_hash": resolved_packet_hash,
        }
    )
    return PB1QReleaseContinuityReviewDecision(
        final_decision=final_decision,
        reason_codes=normalized_reasons,
        release_continuity_review_hash=resolved_packet_hash,
        evidence_hash=resolved_evidence_hash,
...
def _blocked(reason_codes: Sequence[str]) -> PB1QReleaseContinuityReviewDecision:

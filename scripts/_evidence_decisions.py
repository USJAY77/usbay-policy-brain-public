...
DECISIONS = [
    ("approve", "Approve Pilot", "Governance Approved"),
    ("reject", "Reject Pilot", "Rejected"),
    ("human", "Request Human Review", "Human Review Required"),
]
if len(sys.argv) > 1:
    wanted = set(sys.argv[1:])
    DECISIONS = [d for d in DECISIONS if d[0] in wanted]

for action, _, _ in DECISIONS:
    for f in glob.glob(os.path.join(OUT, f"decision_{action}_*.png")):
        os.remove(f)


def section_by_head(page, head):
    for s in page.query_selector_all("#usbpr-body .usbsim-rpt-sec"):
        h = s.query_selector("h4")
        if h and h.inner_text().strip().lower() == head.lower():
            return s
    return None


def drive_to_submitted(page):
...
with sync_playwright() as p:
    ...
    for action, label, expect in DECISIONS:
        page = browser.new_page(viewport={"width": 1440, "height": 2400}, device_scale_factor=2)
        drive_to_submitted(page)
        btn = page.query_selector(f"[data-pr='{action}']")
        assert btn, f"button data-pr={action} not found"
        assert label.lower() in (btn.inner_text() or "").strip().lower(), \
            f"button text mismatch for {action}: {btn.inner_text()!r}"
        btn.click()
        page.wait_for_timeout(700)

        banner = page.query_selector("#usbpr-body .usbpr-banner")
        status_txt = ""
        if banner:
            st = banner.query_selector(".usbpr-status")
            status_txt = (st.inner_text().strip() if st else banner.inner_text().strip())
            banner.scroll_into_view_if_needed()
            time.sleep(0.1)
            banner.screenshot(path=os.path.join(OUT, f"decision_{action}_banner.png"))
        pr_sec = section_by_head(page, "Pilot request")
        if pr_sec:
            pr_sec.scroll_into_view_if_needed()
            time.sleep(0.1)
            pr_sec.screenshot(path=os.path.join(OUT, f"decision_{action}_pilot_request.png"))
        audit = section_by_head(page, "Approval audit trail")
        if audit:
            audit.scroll_into_view_if_needed()
            time.sleep(0.1)
            audit.screenshot(path=os.path.join(OUT, f"decision_{action}_audit_trail.png"))
        page.screenshot(path=os.path.join(OUT, f"decision_{action}_full_modal.png"), full_page=True)
        print(f"[{action}] status={status_txt!r} expect~{expect!r} audit={'yes' if audit else 'no'}")
        page.close()
    browser.close()

print("FILES:")
for f in sorted(glob.glob(os.path.join(OUT, "decision_*.png"))):

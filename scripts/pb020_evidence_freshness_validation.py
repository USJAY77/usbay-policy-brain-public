...
def evaluate_dir(
    ...
    for filename, expected_schema in sorted(expected.items()):
        ...
        if parsed is None:
            ...
            status = "FRESH" if age_hours <= max_age_hours and age_hours >= 0 else "STALE"
            if status == "STALE":
                if artifact_kind(filename) == "certification_result":
                    errors.append(f"PB020_STALE_CERTIFICATION_RESULT:{scope}/{filename}")
                elif artifact_kind(filename) == "maturity_report":
                    errors.append(f"PB020_STALE_MATURITY_REPORT:{scope}/{filename}")
                elif artifact_kind(filename) == "action_tracker":
        ...
        record = {
            ...
            "generated_at": generated_at if isinstance(generated_at, str) else "Information not provided",
            "age_hours": age_hours,

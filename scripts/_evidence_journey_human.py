...
for f in glob.glob(os.path.join(OUT, "journey_7_human.png")):
    os.remove(f)
...
with sync_playwright() as p:
    ...
    rej = page.query_selector("[data-pr='human']")
    assert rej and "human review" in (rej.inner_text() or "").lower()
    rej.click()
    page.wait_for_timeout(700)
    ...
    time.sleep(0.2)
    card.screenshot(path=os.path.join(OUT, "journey_7_human.png"))
    print("captured journey_7_human.png",
          os.path.getsize(os.path.join(OUT, "journey_7_human.png")), "bytes")
    browser.close()

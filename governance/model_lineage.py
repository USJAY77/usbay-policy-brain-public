...
def evaluate_model_lineage(record: dict[str, Any] | None) -> dict[str, Any]:
    reasons: list[str] = []
    if not isinstance(record, dict):
        ...
        if record.get("governance_bypass") is True:
            reasons.append("MODEL_GOVERNANCE_BYPASS")
    clean = sorted(set(str(reason) for reason in reasons if reason))
    return {"model_lineage_status": "VALID" if not clean else "BLOCKED", "reason_codes": clean, "read_only": True}
